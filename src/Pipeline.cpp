...
#define COLOR_ARGB(a, r, g, b)                                              \
    ((uint32)((((a)&0xff) << 24) | (((r)&0xff) << 16) | (((g)&0xff) << 8) | \
              ((b)&0xff)))
...
ObjPipeline::ObjPipeline(uint32 platform) : Pipeline(platform) {
...
void findMinVertAndNumVertices(uint16 *indices, uint32 numIndices,
                               uint32 *minVert, int32 *numVertices) {
    ...
    if (minVert) *minVert = min;
    if (numVertices) *numVertices = num;
}

void instV4d(int type, uint8 *dst, V4d *src, uint32 numVertices,
             uint32 stride) {
    if (type == VERT_FLOAT4)
...
void instV3d(int type, uint8 *dst, V3d *src, uint32 numVertices,
             uint32 stride) {
    if (type == VERT_FLOAT3)
        for (uint32 i = 0; i < numVertices; i++) {
            memcpy(dst, src, 12);
...
void uninstV3d(int type, V3d *dst, uint8 *src, uint32 numVertices,
               uint32 stride) {
    if (type == VERT_FLOAT3)
        for (uint32 i = 0; i < numVertices; i++) {
            memcpy(dst, src, 12);
...
void instTexCoords(int type, uint8 *dst, TexCoords *src, uint32 numVertices,
                   uint32 stride) {
...
void uninstTexCoords(int type, TexCoords *dst, uint8 *src, uint32 numVertices,
                     uint32 stride) {
    assert(type == VERT_FLOAT2);
    for (uint32 i = 0; i < numVertices; i++) {
        memcpy(dst, src, 8);
...
bool32 instColor(int type, uint8 *dst, RGBA *src, uint32 numVertices,
                 uint32 stride) {
    uint8 alpha = 0xFF;
    if (type == VERT_ARGB) {
        for (uint32 i = 0; i < numVertices; i++) {
...
void uninstColor(int type, RGBA *dst, uint8 *src, uint32 numVertices,
                 uint32 stride) {
