...
void defaultBeginUpdateCB(Camera *cam) {
    engine->currentCamera = cam;
    Frame::syncDirty();
    engine->device.beginUpdate(cam);
}

void defaultEndUpdateCB(Camera *cam) {
    engine->device.endUpdate(cam);
}

static void buildPlanes(Camera *cam) {
    ...
    /* Right plane */
    p[2].plane.normal = normalize(cross(v51, sub(c[6], c[5])));
    ...
    /* Top plane */
    p[3].plane.normal = normalize(cross(sub(c[4], c[5]), v51));
    ...
    /* Left plane */
    p[4].plane.normal = normalize(cross(v73, sub(c[4], c[7])));
    ...
    /* Bottom plane */
    p[5].plane.normal = normalize(cross(sub(c[6], c[7]), v73));
...
static void buildClipPersp(Camera *cam) {
...
static void buildClipParallel(Camera *cam) {
    Matrix *ltm = cam->getFrame()->getLTM();
...
static void cameraSync(ObjectWithFrame *obj) {
    /*
...
void worldBeginUpdateCB(Camera *cam) {
    engine->currentWorld = cam->world;
    cam->originalBeginUpdate(cam);
}

void worldEndUpdateCB(Camera *cam) {
    cam->originalEndUpdate(cam);
}

static void worldCameraSync(ObjectWithFrame *obj) {
...
void Camera::clear(RGBA *col, uint32 mode) {
    engine->device.clearCamera(this, col, mode);
}

void Camera::showRaster(void) {
    engine->device.showRaster(this->frameBuffer);
}

void calczShiftScale(Camera *cam) {
...
void Camera::setNearPlane(float32 near) {
    this->nearPlane = near;
    calczShiftScale(this);
    if (this->getFrame()) this->getFrame()->updateObjects();
}

void Camera::setFarPlane(float32 far) {
    this->farPlane = far;
    calczShiftScale(this);
    if (this->getFrame()) this->getFrame()->updateObjects();
}

void Camera::setViewWindow(const V2d *window) {
    this->viewWindow = *window;
    if (this->getFrame()) this->getFrame()->updateObjects();
}

void Camera::setViewOffset(const V2d *offset) {
    this->viewOffset = *offset;
    if (this->getFrame()) this->getFrame()->updateObjects();
}

void Camera::setProjection(int32 proj) {
    this->projection = proj;
    if (this->getFrame()) this->getFrame()->updateObjects();
}

int32 Camera::frustumTestSphere(Sphere *s) {
    int32 res = SPHEREINSIDE;
    FrustumPlane *p = this->frustumPlanes;
    for (int32 i = 0; i < 6; i++) {
        float32 dist = dot(p->plane.normal, s->center) - p->plane.distance;
        if (s->radius < dist) return SPHEREOUTSIDE;
        if (s->radius > -dist) res = SPHEREBOUNDARY;
        p++;
    }
    return res;
}
...
Camera *Camera::streamRead(Stream *stream) {
    CameraChunkData buf;
    ...
    if (s_plglist.streamRead(stream, cam)) return cam;
    cam->destroy();
    return nil;
}

bool Camera::streamWrite(Stream *stream) {
    CameraChunkData buf;
...
uint32 Camera::streamGetSize(void) {
    return 12 + sizeof(CameraChunkData) + 12 + s_plglist.streamGetSize(this);
}

// Assumes horizontal FOV for 4:3, but we convert to vertical FOV
void Camera::setFOV(float32 hfov, float32 ratio) {
    V2d v;
    float w, h;
