...
HAnimHierarchy *HAnimHierarchy::create(int32 numNodes, int32 *nodeFlags,
                                       int32 *nodeIDs, int32 flags,
                                       int32 maxKeySize) {
    HAnimHierarchy *hier =
        (HAnimHierarchy *)rwMalloc(sizeof(*hier), MEMDUR_EVENT | ID_HANIM);
    if (hier == nil) {
    ...
    if (hier->flags & NOMATRICES) {
        ...
        hier->matricesUnaligned =
            rwNew(hier->numNodes * 64 + 0xF, MEMDUR_EVENT | ID_HANIM);
        hier->matrices = (Matrix *)((uintptr)hier->matricesUnaligned & ~0xF);
    }
    hier->nodeInfo =
        rwNewT(HAnimNodeInfo, hier->numNodes, MEMDUR_EVENT | ID_HANIM);
...
void HAnimHierarchy::destroy(void) {
...
static Frame *findUnattachedById(HAnimHierarchy *hier, Frame *f, int32 id) {
    if (f == nil) return nil;
    HAnimData *hanim = HAnimData::get(f);
...
void HAnimHierarchy::attachByIndex(int32 idx) {
...
void HAnimHierarchy::attach(void) {
    for (int32 i = 0; i < this->numNodes; i++) this->attachByIndex(i);
}

int32 HAnimHierarchy::getIndex(int32 id) {
    for (int32 i = 0; i < this->numNodes; i++)
        if (this->nodeInfo[i].id == id) return i;
    return -1;
}

int32 HAnimHierarchy::getIndex(Frame *f) {
    for (int32 i = 0; i < this->numNodes; i++)
        if (this->nodeInfo[i].frame == f) return i;
    return -1;
}

HAnimHierarchy *HAnimHierarchy::get(Frame *f) {
    return HAnimData::get(f)->hierarchy;
}

HAnimHierarchy *HAnimHierarchy::find(Frame *f) {
    if (f == nil) return nil;
...
void HAnimHierarchy::updateMatrices(void) {
    ...
    for (i = 0; i < this->numNodes; i++) {
        ...
        if (node->flags & PUSH) *sp++ = parentMat;
        parentMat = curMat;
        if (node->flags & POP) parentMat = *--sp;
...
HAnimData *HAnimData::get(Frame *f) {
    return PLUGINOFFSET(HAnimData, f, hAnimOffset);
}

static void *createHAnim(void *object, int32 offset, int32) {
...
static void *destroyHAnim(void *object, int32 offset, int32) {
    HAnimData *hanim = PLUGINOFFSET(HAnimData, object, offset);
    if (hanim->hierarchy) hanim->hierarchy->destroy();
    hanim->id = -1;
    hanim->hierarchy = nil;
    return object;
}

static void *copyHAnim(void *dst, void *src, int32 offset, int32) {
...
static Stream *readHAnim(Stream *stream, int32, void *object, int32 offset,
                         int32) {
    int32 ver, numNodes;
    HAnimData *hanim = PLUGINOFFSET(HAnimData, object, offset);
    ver = stream->readI32();
    ...
    if (numNodes != 0) {
        ...
        int32 *nodeFlags = rwNewT(int32, numNodes, MEMDUR_FUNCTION | ID_HANIM);
        int32 *nodeIDs = rwNewT(int32, numNodes, MEMDUR_FUNCTION | ID_HANIM);
        for (int32 i = 0; i < numNodes; i++) {
            nodeIDs[i] = stream->readI32();
            stream->readI32();  // index...unused
            nodeFlags[i] = stream->readI32();
        }
        hanim->hierarchy = HAnimHierarchy::create(numNodes, nodeFlags, nodeIDs,
                                                  flags, maxKeySize);
...
static Stream *writeHAnim(Stream *stream, int32, void *object, int32 offset,
                          int32) {
    HAnimData *hanim = PLUGINOFFSET(HAnimData, object, offset);
    stream->writeI32(256);
...
static int32 getSizeHAnim(void *object, int32 offset, int32) {
    HAnimData *hanim = PLUGINOFFSET(HAnimData, object, offset);
    if (!hAnimDoStream ||
        version >= 0x35000 && hanim->id == -1 && hanim->hierarchy == nil)
        return 0;
    if (hanim->hierarchy) return 12 + 8 + hanim->hierarchy->numNodes * 12;
    return 12;
}

static void hAnimFrameRead(Stream *stream, Animation *anim) {
...
static void hAnimFrameWrite(Stream *stream, Animation *anim) {
    HAnimKeyFrame *frames = (HAnimKeyFrame *)anim->keyframes;
    for (int32 i = 0; i < anim->numFrames; i++) {
...
static uint32 hAnimFrameGetSize(Animation *anim) {
...
static void hanimApplyCB(void *result, void *frame) {
...
static void hanimInterpCB(void *vout, void *vin1, void *vin2, float32 t,
                          void *) {
...
static void *hanimOpen(void *object, int32 offset, int32 size) {
    AnimInterpolatorInfo *info =
        rwNewT(AnimInterpolatorInfo, 1, MEMDUR_GLOBAL | ID_HANIM);
    info->id = 1;
...
static void *hanimClose(void *object, int32 offset, int32 size) {
    return object;
}
...
void registerHAnimPlugin(void) {
    ...
    hAnimOffset = Frame::registerPlugin(sizeof(HAnimData), ID_HANIM,
                                        createHAnim, destroyHAnim, copyHAnim);
