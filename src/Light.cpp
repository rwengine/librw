...
static void lightSync(ObjectWithFrame *) {
}

static void worldLightSync(ObjectWithFrame *obj) {
    Light *light = (Light *)obj;
    light->originalSync(obj);
}

Light *Light::create(int32 type) {
...
void Light::setAngle(float32 angle) {
    this->minusCosAngle = -cos(angle);
}

float32 Light::getAngle(void) {
...
Light *Light::streamRead(Stream *stream) {
    uint32 version;
    LightChunkData buf;
    ...
    if (s_plglist.streamRead(stream, light)) return light;
    light->destroy();
    return nil;
}

bool Light::streamWrite(Stream *stream) {
    LightChunkData buf;
...
uint32 Light::streamGetSize(void) {
