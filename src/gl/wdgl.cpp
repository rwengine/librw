...
void registerPlatformPlugins(void) {
    Driver::registerPlugin(PLATFORM_WDGL, 0, PLATFORM_WDGL, driverOpen,
                           driverClose);
}
...
void uploadGeo(Geometry *geo) {
    ...
    glBufferData(GL_ARRAY_BUFFER, inst->dataSize, inst->data, GL_STATIC_DRAW);

    glGenBuffers(1, &inst->ibo);
    glBindBuffer(GL_ARRAY_BUFFER, inst->ibo);
    glBufferData(GL_ARRAY_BUFFER, meshHeader->totalIndices * 2, 0,
                 GL_STATIC_DRAW);
    GLintptr offset = 0;
...
void setAttribPointers(InstanceDataHeader *inst) {
    static GLenum attribType[] = {GL_FLOAT, GL_BYTE, GL_UNSIGNED_BYTE, GL_SHORT,
                                  GL_UNSIGNED_SHORT};
...
void packattrib(uint8 *dst, float32 *src, AttribDesc *a, float32 scale = 1.0f) {
    int8 *i8dst;
    uint16 *u16dst;
    int16 *i16dst;
...
void unpackattrib(float *dst, uint8 *src, AttribDesc *a, float32 scale = 1.0f) {
    int8 *i8src;
    uint16 *u16src;
    int16 *i16src;
...
Stream *readNativeData(Stream *stream, int32, void *object, int32, int32) {
    ...
    stream->read(header->attribs, header->numAttribs * sizeof(AttribDesc));
...
Stream *writeNativeData(Stream *stream, int32, void *object, int32, int32) {
    Geometry *geometry = (Geometry *)object;
    if (geometry->instData == nil ||
        geometry->instData->platform != PLATFORM_WDGL)
        return stream;
...
void printPipeinfo(Atomic *a) {
    ...
    if (g->instData == nil || g->instData->platform != PLATFORM_WDGL) return;
    int32 plgid = 0;
    if (a->pipeline) plgid = a->pipeline->pluginID;
...
static void instance(rw::ObjPipeline *rwpipe, Atomic *atomic) {
    ...
    if (geo->flags & Geometry::PRELIT) header->numAttribs++;
    if (geo->flags & Geometry::NORMALS) header->numAttribs++;
    int32 offset = 0;
    header->attribs =
        rwNewT(AttribDesc, header->numAttribs, MEMDUR_EVENT | ID_GEOMETRY);
    ...
    if (pipe->instanceCB)
        ...
        header->data =
            rwNewT(uint8, header->dataSize, MEMDUR_EVENT | ID_GEOMETRY);
    }

    a = header->attribs;
    for (int32 i = 0; i < header->numAttribs; i++) a[i].stride = offset;
...
static void uninstance(rw::ObjPipeline *rwpipe, Atomic *atomic) {
    ...
    if (pipe->uninstanceCB) pipe->uninstanceCB(geo);
...
uint32 skinInstanceCB(Geometry *g, int32 i, uint32 offset) {
...
void skinUninstanceCB(Geometry *geo) {
    InstanceDataHeader *header = (InstanceDataHeader *)geo->instData;
...
void initSkin(void) {
    Driver::registerPlugin(PLATFORM_WDGL, 0, ID_SKIN, skinOpen, skinClose);
}
...
void initMatFX(void) {
    Driver::registerPlugin(PLATFORM_WDGL, 0, ID_MATFX, matfxOpen, matfxClose);
}
...
void registerNativeRaster(void) {
    nativeRasterOffset = Raster::registerPlugin(
        sizeof(GlRaster), ID_RASTERWDGL, createNativeRaster,
...
void Texture::upload(void) {
    ...
    static GLenum filternomip[] = {0,         GL_NEAREST, GL_LINEAR, GL_NEAREST,
    ...
    static GLenum wrap[] = {0, GL_REPEAT, GL_MIRRORED_REPEAT, GL_CLAMP_TO_EDGE,
                            GL_CLAMP_TO_BORDER};
    ...
    switch (r->format & 0xF00) {
        case Raster::C8888:
            glTexImage2D(GL_TEXTURE_2D, 0, 4, r->width, r->height, 0, GL_RGBA,
                         GL_UNSIGNED_BYTE, r->pixels);
            break;
        default:
