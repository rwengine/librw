...
static void setAlphaTest(bool32 enable) {
    ...
    if (rwStateCache.alphaTestEnable != enable) {
        ...
        shaderfunc =
            rwStateCache.alphaTestEnable ? rwStateCache.alphaFunc : ALPHAALWAYS;
...
static GLint addressConvMap[] = {0, GL_REPEAT, GL_MIRRORED_REPEAT, GL_CLAMP,
                                 GL_CLAMP_TO_BORDER};
...
static void setFilterMode(uint32 stage, int32 filter) {
    if (rwStateCache.texstage[stage].filter != (Texture::FilterMode)filter) {
        ...
        if (raster) {
            Gl3Raster *natras =
                PLUGINOFFSET(Gl3Raster, rwStateCache.texstage[stage].raster,
                             nativeRasterOffset);
            if (natras->filterMode != filter) {
                setActiveTexture(stage);
                glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER,
                                filterConvMap_NoMIP[filter]);
...
static void setAddressU(uint32 stage, int32 addressing) {
    if (rwStateCache.texstage[stage].addressingU !=
        ...
        if (raster) {
            Gl3Raster *natras =
                PLUGINOFFSET(Gl3Raster, raster, nativeRasterOffset);
...
static void setRasterStageOnly(uint32 stage, Raster *raster) {
    ...
    if (raster != rwStateCache.texstage[stage].raster) {
        ...
        if (raster) {
            ...
            rwStateCache.texstage[stage].filter =
                (rw::Texture::FilterMode)natras->filterMode;
            rwStateCache.texstage[stage].addressingU =
                (rw::Texture::Addressing)natras->addressU;
            rwStateCache.texstage[stage].addressingV =
                (rw::Texture::Addressing)natras->addressV;
    ...
    switch (state) {
        ...
        case SRCBLEND:
            if (rwStateCache.srcblend != value) {
                ...
                glBlendFunc(blendMap[rwStateCache.srcblend],
                            blendMap[rwStateCache.destblend]);
            }
            break;
        case DESTBLEND:
        ...
        case CULLMODE:
            if (rwStateCache.cullmode != value) {
                ...
                if (rwStateCache.cullmode == CULLNONE)
                    ...
                    glCullFace(rwStateCache.cullmode == CULLBACK ? GL_BACK
                                                                 : GL_FRONT);
                }
            }
            break;
...
void setAmbientLight(RGBAf *amb) {
...
void setLight(int32 n, Light *light) {
    UniformLight *l;
    Frame *f;
    Matrix *m;
...
static void clearCamera(Camera *cam, RGBA *col, uint32 mode) {
    ...
    mask = 0;
    if (mode & Camera::CLEARIMAGE) mask |= GL_COLOR_BUFFER_BIT;
    if (mode & Camera::CLEARZ) mask |= GL_DEPTH_BUFFER_BIT;
    glClear(mask);
}
...
#ifdef LIBRW_SDL2
static int openSDL2(EngineStartParams *startparams) {
    if (!startparams) {
    ...
    if (startparams->fullscreen) flags |= SDL_WINDOW_FULLSCREEN;
    win = SDL_CreateWindow(startparams->windowtitle, SDL_WINDOWPOS_UNDEFINED,
                           SDL_WINDOWPOS_UNDEFINED, startparams->width,
                           startparams->height, flags);
    if (win == nil) {
        RWERROR((ERR_ENGINEOPEN, SDL_GetError()));
...
#else
static int openGLFW(EngineStartParams *startparams) {
    GLenum status;
    GLFWwindow *win;
    ...
    win = glfwCreateWindow(startparams->width, startparams->height,
                           startparams->windowtitle, 0, 0);
    if (win == nil) {
    ...
    if (startparams->fullscreen) {
        ...
        if (nbmonitors) {
            ...
            glfwSetWindowMonitor(win, monitors[0], 0, 0, mode->width,
...
static int termOpenGL(void) {
    closeIm3D();
    closeIm2D();
    return 1;
}

static int finalizeOpenGL(void) {
    return 1;
}
