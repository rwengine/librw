...
int32 registerUniform(const char *name) {
    ...
    if (i >= 0) return i;
    // TODO: print error
    if (uniformRegistry.numUniforms + 1 >= MAX_UNIFORMS) return -1;
...
int32 findUniform(const char *name) {
    ...
    for (i = 0; i < uniformRegistry.numUniforms; i++)
        if (strcmp(name, uniformRegistry.uniformNames[i]) == 0) return i;
    return -1;
}
...
int32 registerBlock(const char *name) {
    ...
    if (i >= 0) return i;
    // TODO: print error
    if (uniformRegistry.numBlocks + 1 >= MAX_BLOCKS) return -1;
...
int32 findBlock(const char *name) {
    ...
    for (i = 0; i < uniformRegistry.numBlocks; i++)
        if (strcmp(name, uniformRegistry.blockNames[i]) == 0) return i;
    return -1;
}
...
static int compileshader(GLenum type, const char *src, GLuint *shader) {
    GLint shdr, success;
    GLint len;
    char *log;
...
static int linkprogram(GLint vs, GLint fs, GLuint *program) {
    GLint prog, success;
    GLint len;
    char *log;
...
Shader *Shader::fromStrings(const char *vsrc, const char *fsrc) {
    ...
    for (i = 0; i < uniformRegistry.numBlocks; i++) {
        int idx =
            glGetUniformBlockIndex(program, uniformRegistry.blockNames[i]);
        if (idx >= 0) glUniformBlockBinding(program, idx, i);
    }

    // query uniform locations
    sh->program = program;
    sh->uniformLocations =
        rwNewT(GLint, uniformRegistry.numUniforms, MEMDUR_EVENT | ID_DRIVER);
    for (i = 0; i < uniformRegistry.numUniforms; i++)
        sh->uniformLocations[i] =
            glGetUniformLocation(program, uniformRegistry.uniformNames[i]);

    return sh;
}

Shader *Shader::fromFiles(const char *vspath, const char *fspath) {
    char *vsrc, *fsrc;
