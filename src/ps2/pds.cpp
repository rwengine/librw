...
Pipeline *getPDSPipe(uint32 data) {
    for (int32 i = 0; i < pdsGlobals.numPipes; i++)
        if (pdsGlobals.pipes[i]->pluginData == data) return pdsGlobals.pipes[i];
    return nil;
}
...
void registerPDSPipe(Pipeline *pipe) {
    if (pdsGlobals.pipes == nil)
        pdsGlobals.pipes =
            rwNewT(Pipeline *, pdsGlobals.maxPipes, MEMDUR_GLOBAL | ID_PDS);
    assert(pdsGlobals.numPipes < pdsGlobals.maxPipes);
    pdsGlobals.pipes[pdsGlobals.numPipes++] = pipe;
}

static void atomicPDSRights(void *object, int32, int32, uint32 data) {
    Atomic *a = (Atomic *)object;
    a->pipeline = (ObjPipeline *)getPDSPipe(data);
    //	printf("atm pds: %x %x %x\n", data, a->pipeline->pluginID,
    //a->pipeline->pluginData);
}

static void materialPDSRights(void *object, int32, int32, uint32 data) {
    Material *m = (Material *)object;
    m->pipeline = (ObjPipeline *)getPDSPipe(data);
    //	printf("mat pds: %x %x %x\n", data, m->pipeline->pluginID,
    //m->pipeline->pluginData);
}

static void *pdsOpen(void *object, int32 offset, int32 size) {
    return object;
}
static void *pdsClose(void *object, int32 offset, int32 size) {
