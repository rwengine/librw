...
static void transferMinSize(int32 psm, int32 flags, int32 *minw, int32 *minh) {
    *minh = 1;
    switch (psm) {
        case PSMCT32:
        case PSMZ32:
...
void *mallocalign(size_t size, int32 alignment) {
    void *p;
    void **pp;
    p = rwMalloc(size + alignment + sizeof(void *),
                 MEMDUR_EVENT | ID_RASTERPS2);
    if (p == nil) return nil;
    pp =
        (void **)(((uintptr)p + sizeof(void *) + alignment) & ~(alignment - 1));
    pp[-1] = p;
    return (void *)pp;
}
...
int32 getRasterFormat(Raster *raster) {
    ...
    switch (raster->type) {
        ...
        case Raster::TEXTURE:
            ...
            if (pixelformat == 0) switch (raster->depth) {
                    case 4:
...
static uint8 blockmaprev_PSMCT16[32] = {
    ...
    16, 20, 17, 21, 24, 28, 25, 29, 18, 22, 19, 23, 26, 30, 27, 31,
};
...
static void calcOffsets(int32 width_Px, int32 height_Px, int32 psm,
                        uint64 *bufferBase_B, uint64 *bufferWidth_W,
                        uint32 *trxpos, uint32 *totalSize,
                        uint32 *paletteBase) {
    ...
    for (nlevels = 1; mindim_Px > 8; nlevels++) {
        if (nlevels >= maxMipLevels) break;
        mindim_Px /= 2;
    }

#define PAGEWIDTH_B \
    (pageWidth_Px / blockWidth_Px)  // number of horizontal blocks per page
...
#define REALHEIGHT(w) (max((w), blockHeight_Px))
    ...
    for (n = 1; n < nlevels; n++) {
        ...
        if (lastmipw_Px >= pageWidth_Px) {
            if (lastmiph_Px >= pageHeight_Px) {
                ...
                // This is the only place bufferWidth can change. Similarly
                // bufferBase_2, which is related
                bufferBase_B[n] =
                    bufferBase_B[n - 1] + (lastmipw_Px / blockWidth_Px) *
                                              (lastmiph_Px / blockHeight_Px);
                ...
                // How do we know pageHeight - yoff - REALHEIGHT(lastmiph) >=
                // miph? And how is this condition ever false? Assuming lastmipw
                // >= pageWidth for any number of levels, lastmiph must be
                // pageHeight/2 or lower to reach this code. No dimension is
                // lower than 8. Then consequent mipmaps will have heights
                // halved but even with PSMT4 we will only (vertically) fill the
                ...
                if (REALHEIGHT(lastmiph_Px) + yoff_Px < pageHeight_Px) {
                    ...
                    bufferBase_B[n] = bufferBase_B[n - 1] +
                                      PAGEWIDTH_B * NBLKY(lastmiph_Px) *
                                          bufferWidth_W[n] * 64 /
                                          pageWidth_Px;  // number of horizontal
                                                         // pages for level
                } else {
                    // CASE 3
                    // Can this happen?
            ...
            if (REALWIDTH(lastmipw_Px) + xoff_Px < pageWidth_Px) {
                ...
                basestack_B[sp] =
                    bufferBase_B[n - 1] + NBLKY(lastmiph_Px) * PAGEWIDTH_B;
                sp++;
                xoff_Px += REALWIDTH(lastmipw_Px);
                bufferBase_B[n] = bufferBase_B[n - 1] + NBLKX(lastmipw_Px);
            } else if (sp) {
                ...
                yoff_Px += REALHEIGHT(lastmiph_Px);
                bufferBase_B[n] =
                    bufferBase_B[n - 1] + PAGEWIDTH_B * NBLKY(lastmiph_Px);
            }
        } else {
            // CASE 11
            bufferHeight_P[n] = bufferHeight_P[n - 1];
            ...
            if (REALWIDTH(lastmipw_Px) + xoff_Px < bufferWidth_W[n - 1] * 64) {
                ...
                xoff_Px += REALWIDTH(lastmipw_Px);
                bufferBase_B[n] = bufferBase_B[n - 1] + NBLKX(lastmipw_Px);
            } else if (REALHEIGHT(lastmiph_Px) + yoff_Px <
                           pageHeight_Px * bufferHeight_P[n] &&
                       flag == 0) {
                // CASE 13
                bufferBase_B[n] =
                    bufferBase_B[n - 1] + PAGEWIDTH_B * NBLKY(lastmiph_Px);
                yoff_Px += blockHeight_Px ? lastmiph_Px
                                          : 0;  // how exactly can blockHeight
                                                // be 0?? This looks wrong...
                flag = n;
            } else {
                // CASE 14
                bufferBase_B[n] = basestack_B[sp - 1];
    ...
    if (bufwidth_Px != lastmipw_Px || bufheight_Px != lastmiph_Px) {
        if (psm == PSMT8) {
            ...
            paletteBase_B =
                bufferPage_B[nlevels - 1] +
                ((bufwidth_Px / pageWidth_Px) * bufferHeight_P[nlevels - 1]
    ...
    for (n = 0; n < nlevels; n++) {
        ...
        if (bufwidth_W * 64 / pageWidth_Px > 1) {
            uint32 bufpagestride_B =
                bufwidth_W * 64 * 32 / pageWidth_Px;  // one row of pages
            uint32 bufwidth_B =
                bufwidth_W * 64 / blockWidth_Px;  // one row of blocks
            ...
            bufferBase_B[n] =
                (bufferBase_B[n] &
                 ~((uint64)bufpagestride_B - PAGEWIDTH_B))  // mask out R and P
                | ((bufferBase_B[n] & (bufwidth_B - PAGEWIDTH_B)) *
                   (bufpagestride_B / bufwidth_B))  // extract P and shift left
                | ((bufferBase_B[n] & (bufpagestride_B - bufwidth_B)) /
                   (bufwidth_B / PAGEWIDTH_B));  // extract R and shift right
        }

        // Always have to swizzle blocks inside pages. We use a lookup, RW does
        // bit operations
        switch (psm) {
            case PSMCT32:
            case PSMCT24:
            ...
            case PSMT4HH:
                ...
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMCT32[bufferBase_B[n] & 0x1F];
                break;
            case PSMT4:
            case PSMCT16:
                // ABCDE -> ADBEC
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMCT16[bufferBase_B[n] & 0x1F];
                break;
            case PSMCT16S:
                // ABCDE -> DBAEC
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMCT16S[bufferBase_B[n] & 0x1F];
                break;
            case PSMZ32:
            case PSMZ24:
                // ABCDE -> ~C~ADBE
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMZ32[bufferBase_B[n] & 0x1F];
                break;
            case PSMZ16:
                // ABCDE -> ~A~DBEC
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMZ16[bufferBase_B[n] & 0x1F];
                break;
            case PSMZ16S:
                // ABCDE -> ~D~BAEC
                bufferBase_B[n] =
                    bufferBase_B[n] & ~0x1F |
                    (uint64)blockmap_PSMZ16S[bufferBase_B[n] & 0x1F];
                break;
            default:
                break;
        }
    ...
    if (bufwidth_W * 64 / pageWidth_Px > 1) {
        ...
        paletteBase_B =
            (paletteBase_B &
             ~(bufpagestride_B - PAGEWIDTH_B))  // mask out R and P
            | ((paletteBase_B & (bufwidth_B - PAGEWIDTH_B)) *
               (bufpagestride_B / bufwidth_B))  // extract P and shift left
            | ((paletteBase_B & (bufpagestride_B - bufwidth_B)) /
               (bufwidth_B / PAGEWIDTH_B));  // extract R and shift right
    }
    switch (psm) {
        case PSMCT32:
        ...
        case PSMT4HL:
        case PSMT4HH:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMCT32[paletteBase_B & 0x1F];
            break;
        case PSMT4:
        case PSMCT16:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMCT16[paletteBase_B & 0x1F];
            break;
        case PSMCT16S:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMCT16S[paletteBase_B & 0x1F];
            break;
        case PSMZ32:
        case PSMZ24:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMZ32[paletteBase_B & 0x1F];
            break;
        case PSMZ16:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMZ16[paletteBase_B & 0x1F];
            break;
        case PSMZ16S:
            paletteBase_B = paletteBase_B & ~0x1F |
                            (uint64)blockmap_PSMZ16S[paletteBase_B & 0x1F];
            break;
        default:
            break;
    }
    ...
    *totalSize =
        bufferPage_B[nlevels - 1] +  // start of last buffer`
        bufferWidth_W[nlevels - 1] * 64 /
            blockWidth_Px *  // number of horizontal blocks in last level
            pageHeight_Px * bufferHeight_P[nlevels - 1] /
            blockHeight_Px;  // number of vertical blocks in last level
...
static void createTexRaster(Raster *raster) {
    ...
    if (raster->flags & Raster::DONTALLOCATE) return;
    ...
    int32 s;
    tw = 0;
    for (s = 1; s < width; s *= 2) tw++;
    th = 0;
    for (s = 1; s < height; s *= 2) th++;
    ...
    if (raster->format & Raster::MIPMAP) {
        ...
        for (n = 0; mipw != 0 && miph != 0 && n < maxMipLevels; n++) {
            if (width >= 8 && height >= 8 && (mipw < 8 || miph < 8)) break;
            ...
            if (bufferWidth[n] != lastBufferWidth) {
                ...
                nextaddress =
                    (lastaddress + nPagW * nPagH * WD2PG) & ~(WD2PG - 1);
            }
            lastBufferWidth = bufferWidth[n];
            nextaddress =
                ALIGN64(nextaddress);  // this should already be the case...
            uint32 b = nextaddress >> (11 - 3) &
                       7;  // upper three bits of block-in-page address
            switch (psm) {
                case PSMCT32:
                case PSMCT24:
                case PSMT8:
            ...
            // shift to page address, then to block address and add offset
            // inside page
        ...
        uint32 paletteBase;
        uint32 totalSize;
        calcOffsets(width, height, psm, bufferBase, bufferWidth, trxpos_hi,
                    &totalSize, &paletteBase);

        ras->paletteSize = paletteWidth * paletteHeight * paletteDepth;
        ras->miptbp1 = bufferWidth[1] << 14 | (bufferBase[1] & 0x3FFF) << 0 |
                       bufferWidth[2] << 34 | (bufferBase[2] & 0x3FFF) << 20 |
                       bufferWidth[3] << 54 | (bufferBase[3] & 0x3FFF) << 40;
        ras->miptbp2 = bufferWidth[4] << 14 | (bufferBase[4] & 0x3FFF) << 0 |
        ...
        if (ras->paletteSize) {
            ...
            if (pageWidth * nPagW > width || pageHeight * nPagH > height)
                ...
                nPagW =
                    (paletteWidth + palettePagewidth - 1) / palettePagewidth;
                nPagH =
                    (paletteHeight + palettePageheight - 1) / palettePageheight;
                ras->totalSize += nPagW * nPagH * WD2PG;
            }
        } else
            ras->paletteBase = 0;
    }
    ras->tex0 = tbw << 14 | psm << 20 | tw << 26 | th << 30 | tcc << 34 |
                cpsm << 51 | 0ULL << 55 |  // csm0
    ...
    if (noNewStyleRasters ||
        ...
        ras->data = data;
        raster->pixels = data;
        if (ras->paletteSize) raster->palette = data + ras->pixelSize;
        if (raster->depth == 8) ras->flags |= Ps2Raster::SWIZZLED8;
    } else {
        ...
        if (psm == PSMT8) {
            ...
            if (cpsm == PSMCT32 &&
                bufferWidth[numLevels - 1] == 2) {  // one page
                // unswizzle the starting block of the last buffer and palette
                uint32 bufbase_B =
                    bufferBase[numLevels - 1] & ~0x1F |
                    (uint64)
                        blockmaprev_PSMCT32[bufferBase[numLevels - 1] & 0x1F];
                uint32 palbase_B =
                    ras->paletteBase & ~0x1F |
                    (uint64)blockmaprev_PSMCT32[ras->paletteBase & 0x1F];
                // find start of page of last level (16,16 are PSMT8 block
                // dimensions)
                uint32 page_B = bufbase_B - 8 * (dsay / 16) - dsax / 16;
                // find palette DSAX/Y (in PSMCT32!)
                dsay = (palbase_B - page_B) / 8 *
                       8;  // block/blocksPerPageX * blockHeight
                dsax = (palbase_B - page_B) * 8 %
                       64;  // block*blockWidth % pageWidth
                if (dsay < 0x800) paltrxpos = dsay << 16 | dsax;
            }
        }
        if (psm == PSMT4) {
            ...
            if (rw::version > 0x31000) {
                ...
                if (cpsm == PSMCT16) {
                    // unswizzle the starting block of the last buffer and
                    // palette
                    uint32 bufbase_B =
                        bufferBase[numLevels - 1] & ~0x1F |
                        (uint64)blockmaprev_PSMCT16[bufferBase[numLevels - 1] &
                                                    0x1F];
                    uint32 palbase_B =
                        ras->paletteBase & ~0x1F |
                        (uint64)blockmaprev_PSMCT16[ras->paletteBase & 0x1F];
                    // find start of page of last level (32,16 are PSMT4 block
                    // dimensions)
                    uint32 page_B = bufbase_B - 4 * (dsay / 32) - dsax / 16;
                    // find palette DSAX/Y (in PSMCT16!)
                    dsay = (palbase_B - page_B) / 4 *
                           8;  // block/blocksPerPageX * blockHeight
                    dsax = (palbase_B - page_B) * 16 %
                           128;  // block*blockWidth % pageWidth
                    if (dsay < 0x800) paltrxpos = dsay << 16 | dsax;
                }
            }
        }
        ...
        if (ras->paletteSize) {
            if (rw::version > 0x31000 && paletteHeight == 2) paletteHeight = 3;
            ras->paletteSize =
                0x50 + paletteDepth * paletteWidth * paletteHeight;
        }
        ...
        if (ras->paletteSize && paltrxpos == 0)
            ras->dataSize = ALIGN(ras->pixelSize, 128) +
                            ALIGN(ras->paletteSize, 64) + extrasize + 0x70;
        else
            ras->dataSize =
                ALIGN(ras->paletteSize + ras->pixelSize, 64) + extrasize + 0x70;
        uint8 *data = (uint8 *)mallocalign(ras->dataSize, 0x40);
        ...
        for (n = 0; n < numLevels; n++) {
            ...
            *p++ = 0;
            ...
            if (ras->flags & Ps2Raster::SWIZZLED8 && psm == PSMT8 ||
                ...
                *p++ =
                    (trxpos_hi[n] & ~0x10001) / 2;  // divide both DSAX/Y by 2
            } else {
                *p++ = 0;
                *p++ = trxpos_hi[n];
            }
            ...
            if (trxpos_hi[n] == 0) {
                ...
                *xferchain++ = 0;
                ...
                if (ras->flags & Ps2Raster::SWIZZLED8 && psm == PSMT8) {
                    ...
                    *xferchain++ = PSMCT32 << 24 | bufferWidth[n] / 2
                                                       << 16;  // src buffer
                    *xferchain++ = PSMCT32 << 24 | bufferWidth[n] / 2 << 16 |
                                   bufferBase[n];  // dst buffer
                } else if (ras->flags & Ps2Raster::SWIZZLED4 && psm == PSMT4) {
                    // PSMT4 is swizzled to PSMCT16 and dimensions are halved
                    *xferchain++ = PSMCT16 << 24 | bufferWidth[n] / 2
                                                       << 16;  // src buffer
                    *xferchain++ = PSMCT16 << 24 | bufferWidth[n] / 2 << 16 |
                                   bufferBase[n];  // dst buffer
                } else {
                    *xferchain++ = psm << 24 | bufferWidth[n]
                                                   << 16;  // src buffer
                    *xferchain++ = psm << 24 | bufferWidth[n] << 16 |
                                   bufferBase[n];  // dst buffer
                }
                *xferchain++ = 0x50;
                *xferchain++ = 0;
                ...
                *xferchain++ = 0x30000000 | sz + 5;  // DMAref
                // this obviously only works with 32 bit pointers, but it's only
                // needed on the PS2 anyway
                *xferchain++ = (uint32)(uintptr)p - 0x50;
                *xferchain++ = 0;                    // VIF nop
                *xferchain++ = 0x50000000 | sz + 5;  // VIF DIRECT 2 qwords
            } else {
                // Add to existing transfer
                xferchain[-4] = 0x30000000 | (xferchain[-4] & 0xFFFF) + sz +
                                                 5;  // last DMAref
                xferchain[-1] = 0x50000000 | (xferchain[-1] & 0xFFFF) + sz +
                                                 5;  // last DIRECT
            }
        ...
        if (ras->paletteSize) {
            // huh?
            if (paltrxpos) raster->palette = (uint8 *)p + 0x50;
            ...
            *xferchain++ = 0;
            ...
            if (paltrxpos == 0) {
                ...
                *xferchain++ =
                    cpsm << 24 | 1 << 16 | ras->paletteBase;  // dst buffer
                *xferchain++ = 0x50;
                *xferchain++ = 0;
            } else {
                // copy last pixel bitbltbuf...if uploading palette separately
                // it's still the same buffer
                xferchain[0] = xferchain[-16];
                xferchain[1] = xferchain[-15];
                xferchain[2] = xferchain[-14];
                xferchain[3] = xferchain[-13];
                // Add to last transfer
                xferchain[-16] = 0x30000000 | (xferchain[-16] & 0xFFFF) + sz +
                                                  5;  // last DMAref
                xferchain[-13] = 0x50000000 | (xferchain[-13] & 0xFFFF) + sz +
                                                  5;  // last DIRECT
                xferchain += 4;
                pp->numTransfers--;
            }
...
void rasterCreate(Raster *raster) {
    if (!getRasterFormat(raster)) return;
...
uint8 *paletteLock(Raster *raster) {
    if ((raster->format & (Raster::PAL4 | Raster::PAL8)) == 0) return nil;
    return raster->palette;
}

void paletteUnlock(Raster *raster) {
    // TODO
    (void)raster;
}

static void convertCSM1_16(uint16 *dst, uint16 *src) {
...
static void convertCSM1_32(uint32 *dst, uint32 *src) {
    // palette index bits 0x08 and 0x10 are flipped
    int i, j;
    for (i = 0; i < 256; i++) {
        j = i & ~0x18 | (i & 0x10) >> 1 | (i & 0x8) << 1;
...
// Original code from
// http://ps2linux.no-ip.info/playstation2-linux.com/docs/howto/display_docef7c.html?docid=75
static void unswizzle32to8(uint8 *dst, uint8 *src, int32 w, int32 h) {
    int32 x, y;
    for (y = 0; y < h; y++)
        for (x = 0; x < w; x++) {
...
static void unswizzle16to4(uint8 *dst, uint8 *src, int32 w, int32 h) {
    int32 x, y;
    for (y = 0; y < h; y++)
        for (x = 0; x < w; x++) {
            ...
            int32 hword_loc =
                ypos * w + (((x + swap_sel) & 0x7) + ((x & 0x10) >> 1)) * 2;
...
void copyPSMT8(uint8 *dst, uint8 *src, int32 w, int32 h, int32 srcw) {
    int32 x, y;
    for (y = 0; y < h; y++)
        for (x = 0; x < w; x++) dst[y * w + x] = src[y * srcw + x];
}
...
Image *rasterToImage(Raster *raster) {
    ...
    if (depth == 4) {
        if (natras->flags & Ps2Raster::SWIZZLED4) {
            uint8 *tmp =
                rwNewT(uint8, tw * th / 2, MEMDUR_FUNCTION | ID_RASTERPS2);
...
int32 rasterNumLevels(Raster *raster) {
    ...
    if (raster->format & Raster::MIPMAP) return MAXLEVEL(ras) + 1;
    return 1;
}
...
static void *copyNativeRaster(void *dst, void *src, int32 offset, int32) {
...
static Stream *readMipmap(Stream *stream, int32, void *object, int32 offset,
                          int32) {
    uint16 val = stream->readI32();
    Texture *tex = (Texture *)object;
    if (tex->raster == nil) return stream;
    Ps2Raster *raster = PLUGINOFFSET(Ps2Raster, tex->raster, offset);
    raster->kl = val;
    return stream;
}

static Stream *writeMipmap(Stream *stream, int32, void *object, int32 offset,
                           int32) {
    Texture *tex = (Texture *)object;
    if (tex->raster) {
...
static int32 getSizeMipmap(void *, int32, int32) {
    return rw::platform == PLATFORM_PS2 ? 4 : 0;
}

void registerNativeRaster(void) {
    nativeRasterOffset = Raster::registerPlugin(
        sizeof(Ps2Raster), ID_RASTERPS2, createNativeRaster,
        destroyNativeRaster, copyNativeRaster);

    Texture::registerPlugin(0, ID_SKYMIPMAP, nil, nil, nil);
    Texture::registerPluginStream(ID_SKYMIPMAP, readMipmap, writeMipmap,
                                  getSizeMipmap);
}

void printTEX0(uint64 tex0) {
    printf("%016llX ", tex0);
    uint32 tbp0 = tex0 & 0x3FFF;
    tex0 >>= 14;
    uint32 tbw = tex0 & 0x3F;
    tex0 >>= 6;
    uint32 psm = tex0 & 0x3F;
    tex0 >>= 6;
    uint32 tw = tex0 & 0xF;
    tex0 >>= 4;
    uint32 th = tex0 & 0xF;
    ...
    uint32 tfx = tex0 & 0x3;
    tex0 >>= 2;
    uint32 cbp = tex0 & 0x3FFF;
    tex0 >>= 14;
    uint32 cpsm = tex0 & 0xF;
    tex0 >>= 4;
    uint32 csm = tex0 & 0x1;
    tex0 >>= 1;
    uint32 csa = tex0 & 0x1F;
    tex0 >>= 5;
    uint32 cld = tex0 & 0x7;
    printf(
        "TBP0:%4X TBW:%2X PSM:%2X TW:%X TH:%X TCC:%X TFX:%X CBP:%4X CPSM:%X "
        "CSM:%X CSA:%2X CLD:%X\n",
...
void printTEX1(uint64 tex1) {
    ...
    uint32 mxl = tex1 & 0x7;
    tex1 >>= 3;
    uint32 mmag = tex1 & 0x1;
    tex1 >>= 1;
    uint32 mmin = tex1 & 0x7;
    tex1 >>= 3;
    uint32 mtba = tex1 & 0x1;
    tex1 >>= 10;
    uint32 l = tex1 & 0x3;
    tex1 >>= 13;
    uint32 k = tex1 & 0xFFF;
...
void calcTEX1(Raster *raster, uint64 *tex1, int32 filter) {
    ...
    switch (filter) {
        case Texture::NEAREST:
            t1 |= (NEAREST << 5) | (NEAREST << 6);
            break;
        case Texture::LINEAR:
            t1 |= (LINEAR << 5) | (LINEAR << 6);
            break;
        case Texture::MIPNEAREST:
            t1 |= (NEAREST << 5) | (NEAREST_MIPMAP_NEAREST << 6);
            break;
        case Texture::MIPLINEAR:
            t1 |= (LINEAR << 5) | (LINEAR_MIPMAP_NEAREST << 6);
            break;
        case Texture::LINEARMIPNEAREST:
            t1 |= (NEAREST << 5) | (NEAREST_MIPMAP_LINEAR << 6);
            break;
        case Texture::LINEARMIPLINEAR:
            t1 |= (LINEAR << 5) | (LINEAR_MIPMAP_LINEAR << 6);
            break;
    }
    *tex1 = t1;
}
...
Texture *readNativeTexture(Stream *stream) {
    ...
    */
    ...
    rw::version = version;
    raster = Raster::create(streamExt.width, streamExt.height, streamExt.depth,
                            streamExt.rasterFormat, PLATFORM_PS2);
    noNewStyleRasters = 0;
    ...
    //	       natras->miptbp1[0], natras->miptbp1[1], natras->miptbp2[0],
    //natras->miptbp2[1]); 	printTEX0(((uint64)natras->tex0[1] << 32) |
    //natras->tex0[0]);
    uint64 tex1;
    ...
    if (streamExt.type < 2) {
        ...
        stream->read(((Ps2Raster::PixelPtr *)raster->originalPixels)->pixels,
                     natras->pixelSize);
...
void writeNativeTexture(Texture *tex, Stream *stream) {
    ...
    if (ras->flags == Ps2Raster::SWIZZLED8 && raster->depth == 8)
        streamExt.type = 1;
    if (ras->flags & Ps2Raster::NEWSTYLE) streamExt.type = 2;
    ...
    if (streamExt.type < 2) {
        ...
        stream->write(((Ps2Raster::PixelPtr *)raster->originalPixels)->pixels,
                      ras->pixelSize);
