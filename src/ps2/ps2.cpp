...
void registerPlatformPlugins(void) {
    Driver::registerPlugin(PLATFORM_PS2, 0, PLATFORM_PS2, driverOpen,
                           driverClose);
...
Stream *readNativeData(Stream *stream, int32, void *object, int32, int32) {
    Geometry *geometry = (Geometry *)object;
    uint32 platform;
    ...
    for (uint32 i = 0; i < header->numMeshes; i++) {
        ...
        instance->dataRaw = rwNewT(uint8, instance->dataSize + 0x7F,
...
#ifdef RW_PS2
        if (!buf[1]) fixDmaOffsets(instance);
#endif
...
void registerNativeDataPlugin(void) {
    Geometry::registerPlugin(0, ID_NATIVEDATA, nil, destroyNativeData, nil);
    Geometry::registerPluginStream(ID_NATIVEDATA, readNativeData,
                                   writeNativeData, getSizeNativeData);
}
...
bool32 unfixDmaOffsets(InstanceData *inst) {
    bool32 norefs = 1;
#ifdef RW_PS2
    uint32 base = (uint32)inst->data;
#endif
...
PipeAttribute attribXYZ = {"XYZ", AT_V3_32};

PipeAttribute attribXYZW = {"XYZW", AT_V4_32};

PipeAttribute attribUV = {"UV", AT_V2_32};

PipeAttribute attribUV2 = {"UV2", AT_V4_32};

PipeAttribute attribRGBA = {"RGBA", AT_V4_8 | AT_UNSGN};
...
PipeAttribute attribWeights = {"Weights", AT_V4_32 | AT_RW};

static uint32 attribSize(uint32 unpack) {
...
static uint32 getBatchSize(MatPipeline *pipe, uint32 vertCount) {
    PipeAttribute *a;
...
uint32 *instanceXYZ(uint32 *p, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
    ...
    while ((uintptr)p % 0x10) *p++ = 0;
    return p;
}

uint32 *instanceXYZW(uint32 *p, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
    uint32 *d = (uint32 *)g->morphTargets[0].vertices;
...
uint32 *instanceUV(uint32 *p, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
    uint32 *d = (uint32 *)g->texCoords[0];
    if ((g->flags & Geometry::TEXTURED) || (g->flags & Geometry::TEXTURED2))
        for (uint32 i = idx; i < idx + n; i++) {
            j = m->indices[i];
            *p++ = d[j * 2 + 0];
            *p++ = d[j * 2 + 1];
...
uint32 *instanceUV2(uint32 *p, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
...
uint32 *instanceRGBA(uint32 *p, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
    ...
    if ((g->flags & Geometry::PRELIT))
        ...
        for (uint32 i = idx; i < idx + n; i++) *p++ = 0xFF000000;
    while ((uintptr)p % 0x10) *p++ = 0;
    return p;
}

uint32 *instanceNormal(uint32 *wp, Geometry *g, Mesh *m, uint32 idx, uint32 n) {
    uint16 j;
...
MatPipeline::MatPipeline(uint32 platform)
    ...
    , preUninstCB(nil)
    , postUninstCB(nil) {
    for (int i = 0; i < 10; i++) this->attribs[i] = nil;
}

void MatPipeline::dump(void) {
    if (this->platform != PLATFORM_PS2) return;
    PipeAttribute *a;
...
void MatPipeline::setTriBufferSizes(uint32 inputStride, uint32 bufferSize) {
    PipeAttribute *a;
...
InstMeshInfo getInstMeshInfo(MatPipeline *pipe, Geometry *g, Mesh *m) {
    PipeAttribute *a;
    InstMeshInfo im;
    im.numAttribs = 0;
    ...
    if (im.lastBatchVertCount == 0) im.lastBatchVertCount = im.batchVertCount;
    ...
    if (im.numBrokenAttribs == 0)
        ...
        im.size = 2 * im.numBrokenAttribs * im.numBatches +
                  (1 + im.batchSize) * (im.numBatches - 1) + 1 +
                  im.lastBatchSize;
...
void MatPipeline::instance(Geometry *g, InstanceData *inst, Mesh *m) {
    PipeAttribute *a;
    ...
    for (uint32 j = 0; j < im.numBatches; j++) {
        ...
        for (uint i = 0; i < nelem(this->attribs); i++)
            if ((a = this->attribs[i]) && a->attrib & AT_RW) {
                ...
                *p++ = VIF_STCYCL1 | this->inputStride;
                // Round up nverts so UNPACK will fit exactly into the DMA
                // packet
                //  (can't pad with zeroes in broken out sections).
                int num = (QWC(nverts * atsz) << 4) / atsz;
                *p++ = (a->attrib & 0xFF004000) | 0x8000 | num << 16 |
                       i;  // UNPACK
                // This probably shouldn't happen.
                if (num * this->inputStride > this->vifOffset)
                    fprintf(stderr,
                            "WARNING: PS2 instance data over vifOffset %08X, "
                            "%X-> %X %X\n",
                            p[-1], num, num * this->inputStride,
                            this->vifOffset);
        ...
        for (uint i = 0; i < nelem(this->attribs); i++)
            if ((a = this->attribs[i]) && (a->attrib & AT_RW) == 0) {
                ...
                *p++ = VIF_STMOD;
                *p++ = VIF_STCYCL1 | this->inputStride;
                *p++ = (a->attrib & 0xFF004000) | 0x8000 | nverts << 16 |
                       i;  // UNPACK
        ...
        idx += g->meshHeader->flags == 1 ? im.batchVertCount - 2
                                         : im.batchVertCount;
    ...
    if (this->instanceCB) this->instanceCB(this, g, m, datap);
#ifdef RW_PS2
    if (im.numBrokenAttribs) fixDmaOffsets(inst);
#endif
}

uint8 *MatPipeline::collectData(Geometry *g, InstanceData *inst, Mesh *m,
                                uint8 *data[]) {
    PipeAttribute *a;
    InstMeshInfo im = getInstMeshInfo(this, g, m);

    uint8 *raw = rwNewT(uint8, im.vertexSize * m->numIndices,
                        MEMDUR_EVENT | ID_GEOMETRY);
    uint8 *dp = raw;
    for (uint i = 0; i < nelem(this->attribs); i++)
        if (a = this->attribs[i])
    ...
    uint32 *p = (uint32 *)inst->data;
    if (im.numBrokenAttribs == 0) p += 4;
    for (uint32 j = 0; j < im.numBatches; j++) {
        uint32 nverts =
            j < im.numBatches - 1 ? im.batchVertCount : im.lastBatchVertCount;
        for (uint i = 0; i < nelem(this->attribs); i++)
            if ((a = this->attribs[i]) && a->attrib & AT_RW) p += 8;
        if (im.numBrokenAttribs) p += 4;
        for (uint i = 0; i < nelem(this->attribs); i++)
            if ((a = this->attribs[i]) && (a->attrib & AT_RW) == 0) {
...
static void objInstance(rw::ObjPipeline *rwpipe, Atomic *atomic) {
    ...
    for (uint32 i = 0; i < header->numMeshes; i++) {
        ...
        MatPipeline *m;
        m = pipe->groupPipeline ? pipe->groupPipeline
...
static void objUninstance(rw::ObjPipeline *rwpipe, Atomic *atomic) {
    ObjPipeline *pipe = (ObjPipeline *)rwpipe;
    Geometry *geo = atomic->geometry;
    if ((geo->flags & Geometry::NATIVE) == 0) return;
    ...
    uint32 *flags =
        rwNewT(uint32, geo->numVertices, MEMDUR_FUNCTION | ID_GEOMETRY);
    memset(flags, 0, 4 * geo->numVertices);
    memset(geo->meshHeader->getMeshes()->indices, 0,
           2 * geo->meshHeader->totalIndices);
    for (uint32 i = 0; i < header->numMeshes; i++) {
        Mesh *mesh = &geo->meshHeader->getMeshes()[i];
        MatPipeline *m;
        m = pipe->groupPipeline ? pipe->groupPipeline
...
void insertVertex(Geometry *geo, int32 i, uint32 mask, Vertex *v) {
    ...
    if (mask & 0x10) geo->morphTargets[0].normals[i] = v->n;
    if (mask & 0x100) geo->colors[i] = v->c;
    if (mask & 0x1000) geo->texCoords[0][i] = v->t;
    if (mask & 0x2000) geo->texCoords[1][i] = v->t1;
}

void genericPreCB(MatPipeline *pipe, Geometry *geo) {
    PipeAttribute *a;
...
void genericUninstanceCB(MatPipeline *pipe, Geometry *geo, uint32 flags[],
                         Mesh *mesh, uint8 *data[]) {
    ...
    int8 *adc = nil;
    Skin *skin = nil;
    if (skinGlobals.geoOffset) skin = Skin::get(geo);
    ...
    for (int32 i = 0; i < nelem(pipe->attribs); i++)
        if (a = pipe->attribs[i]) {
            if (a == &attribXYZ)
                ...
                normals = (int8 *)data[i];
            else if (a == &attribWeights)
                weights = (uint32 *)data[i];
        }

    uint32 mask = 0x1;  // vertices
    if (normals && geo->flags & Geometry::NORMALS) mask |= 0x10;
    if (rgba && geo->flags & Geometry::PRELIT) mask |= 0x100;
    if ((uv || uv2) && geo->numTexCoordSets > 0) mask |= 0x1000;
    if (uv2 && geo->numTexCoordSets > 1) mask |= 0x2000;
    if (weights && skin) mask |= 0x10000;
    if (xyzw) adc = getADCbitsForMesh(geo, mesh);

    Vertex v;
    for (uint32 i = 0; i < mesh->numIndices; i++) {
        if (mask & 0x1) memcpy(&v.p, xyz ? xyz : xyzw, 12);
        if (mask & 0x10) {
        ...
        if (mask & 0x100) memcpy(&v.c, rgba, 4);
        if (mask & 0x1000) memcpy(&v.t, uv ? uv : uv2, 8);
        if (mask & 0x2000) memcpy(&v.t1, uv2 + 2, 8);
        if (mask & 0x10000)
            for (int j = 0; j < 4; j++) {
        ...
        if (idx < 0) idx = geo->numVertices++;
        mesh->indices[i] = idx;
        if (adc) adc[i] = xyzw[3] != 0.0f;
        flags[idx] = mask;
...
/*
void
defaultUninstanceCB(MatPipeline *pipe, Geometry *geo, uint32 flags[], Mesh
*mesh, uint8 *data[])
{
...
int8 *getADCbits(Geometry *geo) {
    ...
    if (adcOffset) {
        ...
        if (adc->adcFormatted) bits = adc->adcBits;
    }
    return bits;
}
...
// TODO
void convertADC(Geometry *) {
}

// Not optimal but works
void unconvertADC(Geometry *g) {
    ADCData *adc = PLUGINOFFSET(ADCData, g, adcOffset);
    if (!adc->adcFormatted) return;
    int8 *b = adc->adcBits;
...
static void *createADC(void *object, int32 offset, int32) {
    ADCData *adc = PLUGINOFFSET(ADCData, object, offset);
    adc->adcFormatted = 0;
    return object;
}

static void *copyADC(void *dst, void *src, int32 offset, int32) {
    ...
    if (!dstadc->adcFormatted) return dst;
...
static void *destroyADC(void *object, int32 offset, int32) {
    ADCData *adc = PLUGINOFFSET(ADCData, object, offset);
    if (adc->adcFormatted) rwFree(adc->adcBits);
    return object;
}

static Stream *readADC(Stream *stream, int32, void *object, int32 offset,
                       int32) {
    ADCData *adc = PLUGINOFFSET(ADCData, object, offset);
...
static Stream *writeADC(Stream *stream, int32 len, void *object, int32 offset,
                        int32) {
    ADCData *adc = PLUGINOFFSET(ADCData, object, offset);
    Geometry *geometry = (Geometry *)object;
...
static int32 getSizeADC(void *object, int32 offset, int32) {
    Geometry *geometry = (Geometry *)object;
    ADCData *adc = PLUGINOFFSET(ADCData, object, offset);
    if (!adc->adcFormatted) return 0;
    if (geometry->flags & Geometry::NATIVE) return 16;
    return 16 + (adc->numBits + 3 & ~3);
}

void registerADCPlugin(void) {
    adcOffset = Geometry::registerPlugin(sizeof(ADCData), ID_ADC, createADC,
                                         destroyADC, copyADC);
    Geometry::registerPluginStream(ID_ADC, readADC, writeADC, getSizeADC);
}

// misc stuff

static uint32 unpackSize(uint32 unpack) {
    static uint32 size[] = {32, 16, 8, 4};
    return ((unpack >> 26 & 3) + 1) * size[unpack >> 24 & 3] / 8;
}

/* A little dumb VIF interpreter */
static void sendVIF(uint32 w) {
    enum VIFstate {
        VST_cmd,
        VST_stmask,
        VST_strow,
    ...
    if (n == 0) state = VST_cmd;
}

static void dmaVIF(int32 qwc, uint32 *data) {
    qwc *= 4;
    while (qwc--) sendVIF(*data++);
}
