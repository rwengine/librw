...
void initSkin(void) {
    Driver::registerPlugin(PLATFORM_PS2, 0, ID_SKIN, skinOpen, skinClose);
}
...
Stream *readNativeSkin(Stream *stream, int32, void *object, int32 offset) {
    ...
    *PLUGINOFFSET(Skin *, geometry, offset) = skin;
    ...
    if (oldFormat) {
        skin->numWeights = 4;
        for (int32 i = 0; i < skin->numUsedBones; i++) skin->usedBones[i] = i;
    }
...
void instanceSkinData(Geometry *, Mesh *m, Skin *skin, uint32 *data) {
    uint16 j;
...
int32 findVertexSkin(Geometry *g, uint32 flags[], uint32 mask, Vertex *v) {
    ...
    for (int32 i = 0; i < g->numVertices; i++) {
        ...
        if (mask & flag & 0x1 && !equal(*verts, v->p)) goto cont;
        if (mask & flag & 0x10 && !equal(*norms, v->n)) goto cont;
        if (mask & flag & 0x100 && !equal(*cols, v->c)) goto cont;
        if (mask & flag & 0x1000 && !equal(*tex, v->t)) goto cont;
        if (mask & flag & 0x2000 && !equal(*tex1, v->t1)) goto cont;
        if (mask & flag & 0x10000 &&
            !(wghts[0] == v->w[0] && wghts[1] == v->w[1] &&
              wghts[2] == v->w[2] && wghts[3] == v->w[3] &&
              inds[0] == v->i[0] && inds[1] == v->i[1] && inds[2] == v->i[2] &&
              inds[3] == v->i[3]))
            goto cont;
        return i;
    cont:
...
/*
void
skinUninstanceCB(MatPipeline*, Geometry *geo, uint32 flags[], Mesh *mesh, uint8
*data[])
{
...
void skinPreCB(MatPipeline *, Geometry *geo) {
    Skin *skin = Skin::get(geo);
    if (skin == nil) return;
...
void skinPostCB(MatPipeline *, Geometry *geo) {
    Skin *skin = Skin::get(geo);
    if (skin) {
