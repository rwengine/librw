...
enum PS2AttibTypes { AT_XYZ = 0, AT_UV = 1, AT_RGBA = 2, AT_NORMAL = 3 };
...
public:
    ...
    static uint32 getVertCount(uint32 top, uint32 inAttribs, uint32 outAttribs,
                               uint32 outBufs) {
...
// void defaultUninstanceCB(MatPipeline *pipe, Geometry *geo, uint32 flags[],
// Mesh *mesh, uint8 *data[]);
void skinInstanceCB(MatPipeline *, Geometry *g, Mesh *m, uint8 **data);
// void skinUninstanceCB(MatPipeline*, Geometry *geo, uint32 flags[], Mesh
// *mesh, uint8 *data[]);
