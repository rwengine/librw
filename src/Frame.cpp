...
static void *frameOpen(void *object, int32 offset, int32 size) {
    engine->frameDirtyList.init();
    return object;
}
static void *frameClose(void *object, int32 offset, int32 size) {
    return object;
}

void Frame::registerModule(void) {
    Engine::registerPlugin(0, ID_FRAMEMODULE, frameOpen, frameClose);
}

Frame *Frame::create(void) {
...
Frame *Frame::cloneHierarchy(void) {
...
void Frame::destroy(void) {
    ...
    if (this->getParent()) this->removeChild();
    if (this->object.privateFlags & Frame::HIERARCHYSYNC)
        this->inDirtyList.remove();
    for (Frame *f = this->child; f; f = f->next) f->object.parent = nil;
    rwFree(this);
}

void Frame::destroyHierarchy(void) {
    Frame *next;
...
Frame *Frame::addChild(Frame *child, bool32 append) {
    Frame *c;
    if (child->getParent()) child->removeChild();
    if (append) {
        if (this->child == nil)
            this->child = child;
        else {
    ...
    for (c = child->child; c; c = c->next) c->setHierarchyRoot(this);
...
Frame *Frame::forAllChildren(Callback cb, void *data) {
    for (Frame *f = this->child; f; f = f->next) cb(f, data);
    return this;
}

static Frame *countCB(Frame *f, void *count) {
    (*(int32 *)count)++;
    f->forAllChildren(countCB, count);
    return f;
}

int32 Frame::count(void) {
    int32 count = 1;
...
static void syncLTMRecurse(Frame *frame, uint8 hierarchyFlags) {
    for (; frame; frame = frame->next) {
        ...
        if (hierarchyFlags & Frame::SUBTREESYNCLTM) {
            Matrix::mult(&frame->ltm, &frame->matrix, &frame->getParent()->ltm);
...
static void syncObjRecurse(Frame *frame) {
    for (; frame; frame = frame->next) {
...
static void syncRecurse(Frame *frame, uint8 hierarchyFlags) {
    for (; frame; frame = frame->next) {
        // If frame is dirty or any parent was dirty, update LTM
        hierarchyFlags |= frame->object.privateFlags;
        if (hierarchyFlags & Frame::SUBTREESYNCLTM)
...
void Frame::syncHierarchyLTM(void) {
    // Sync root's LTM
...
void Frame::rotate(V3d *axis, float32 angle, CombineOp op) {
    this->matrix.rotate(axis, angle, op);
    updateObjects();
}

void Frame::translate(V3d *trans, CombineOp op) {
    this->matrix.translate(trans, op);
    updateObjects();
}

void Frame::scale(V3d *scl, CombineOp op) {
    this->matrix.scale(scl, op);
    updateObjects();
}

void Frame::transform(Matrix *mat, CombineOp op) {
...
void Frame::setHierarchyRoot(Frame *root) {
    this->root = root;
    for (Frame *child = this->child; child; child = child->next)
        child->setHierarchyRoot(root);
}

// Clone a frame hierarchy. Link cloned frames into Frame::root of the
// originals.
Frame *Frame::cloneAndLink(Frame *clonedroot) {
    Frame *frame = Frame::create();
    if (clonedroot == nil) clonedroot = frame;
...
FrameList_ *FrameList_::streamRead(Stream *stream) {
    FrameStreamData buf;
    this->numFrames = 0;
    this->frames = nil;
    ...
    this->frames = (Frame **)rwMalloc(this->numFrames * sizeof(Frame *),
                                      MEMDUR_EVENT | ID_FRAMELIST);
    if (this->frames == nil) {
...
void FrameList_::streamWrite(Stream *stream) {
    FrameStreamData buf;
    ...
    for (int32 i = 0; i < this->numFrames; i++) {
        ...
        buf.parent =
            findPointer(f->getParent(), (void **)this->frames, this->numFrames);
...
static Frame *sizeCB(Frame *f, void *size) {
    *(int32 *)size += Frame::s_plglist.streamGetSize(f);
    f->forAllChildren(sizeCB, size);
    return f;
}

uint32 FrameList_::streamGetSize(Frame *f) {
...
Frame **makeFrameList(Frame *frame, Frame **flist) {
    *flist++ = frame;
    if (frame->next) flist = makeFrameList(frame->next, flist);
    if (frame->child) flist = makeFrameList(frame->child, flist);
    return flist;
}
