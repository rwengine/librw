...
int vertFormatMap[] = {-1, VERT_FLOAT2, VERT_FLOAT3, -1, VERT_ARGB};

void *createIndexBuffer(uint32 length) {
#ifdef RW_D3D9
    IDirect3DIndexBuffer9 *ibuf;
    d3ddevice->CreateIndexBuffer(length, D3DUSAGE_WRITEONLY, D3DFMT_INDEX16,
                                 D3DPOOL_MANAGED, &ibuf, 0);
    return ibuf;
#else
...
void *createVertexBuffer(uint32 length, uint32 fvf, int32 pool) {
    ...
    d3ddevice->CreateVertexBuffer(length, D3DUSAGE_WRITEONLY, fvf,
                                  (D3DPOOL)pool, &vbuf, 0);
    return vbuf;
#else
    (void)fvf;
...
uint8 *lockVertices(void *vertexBuffer, uint32 offset, uint32 size,
                    uint32 flags) {
    if (vertexBuffer == nil) return nil;
#ifdef RW_D3D9
    uint8 *verts;
...
void *createTexture(int32 width, int32 height, int32 numlevels, uint32 format) {
#ifdef RW_D3D9
    IDirect3DTexture9 *tex;
    d3ddevice->CreateTexture(width, height, numlevels, 0, (D3DFORMAT)format,
                             D3DPOOL_MANAGED, &tex, nil);
    return tex;
#else
    int32 w = width;
    int32 h = height;
...
static void rasterCreateTexture(Raster *raster) {
    uint32 format;
    int32 levels;
    ...
    natras->texture =
        createTexture(raster->width, raster->height,
                      raster->format & Raster::MIPMAP ? levels : 1, format);
}

static void rasterCreateCameraTexture(Raster *raster) {
    if (raster->format & (Raster::PAL4 | Raster::PAL8))
...
#ifdef RW_D3D9
    if (raster->type == Raster::CAMERATEXTURE) removeVidmemRaster(raster);
#endif
    if (natras->texture) deleteObject(natras->texture);
...
void registerNativeRaster(void) {
    nativeRasterOffset = Raster::registerPlugin(
        sizeof(D3dRaster), ID_RASTERD3D9, createNativeRaster,
