...
int32 getSizeNativeSkin(void *object, int32 offset) {
    ...
    if (skin->platformData == nil) return -1;
    NativeSkin *natskin = (NativeSkin *)skin->platformData;
...
void skinInstanceCB(Geometry *geo, InstanceDataHeader *header) {
    ...
    uint8 *vbuf = rwNewT(uint8, header->numVertices * natskin->stride,
                         MEMDUR_EVENT | ID_SKIN);
    ...
    while (numVertices--) {
        ...
        w[0] = 255 - sum;
        for (int i = 0; i < skin->numWeights; i++) p[i] = w[i];
        p += natskin->stride;
        weights += 4;
    }
...
void skinUninstanceCB(Geometry *geo, InstanceDataHeader *header) {
    ...
    while (numVertices--) {
        ...
        for (int32 j = 0; j < skin->numWeights; j++) w[j] = *p++ / 255.0f;
...
void initSkin(void) {
    Driver::registerPlugin(PLATFORM_XBOX, 0, ID_SKIN, skinOpen, skinClose);
}
