...
void lightingCB(bool32 normals) {
    ...
    FORLIST(lnk, world->directionalLights) {
        ...
        if ((l->getFlags() & Light::LIGHTATOMICS) == 0) continue;
        if (normals && l->getType() == Light::DIRECTIONAL &&
    ...
    for (; n < MAX_LIGHTS; n++) d3ddevice->LightEnable(n, FALSE);
    convColor(&amb, &ambLight);
    d3d::setRenderState(D3DRS_AMBIENT,
                        D3DCOLOR_RGBA(amb.red, amb.green, amb.blue, amb.alpha));
}
