...
void registerPlatformPlugins(void) {
    Driver::registerPlugin(PLATFORM_D3D9, 0, PLATFORM_D3D9, driverOpen,
                           driverClose);
    // shared between D3D8 and 9
    if (nativeRasterOffset == 0) registerNativeRaster();
}
...
uint32 getDeclaration(void *declaration, VertexElement *elements) {
    ...
    while (e[n++].stream != 0xFF)
        ;
    if (elements) memcpy(elements, declaration, n * sizeof(VertexElement));
    return n;
#endif
}
...
Stream *readNativeData(Stream *stream, int32, void *object, int32, int32) {
    ...
    uint8 *p = data;
    header->serialNumber = *(uint32 *)p;
    p += 4;
    header->numMeshes = *(uint32 *)p;
    p += 4;
    header->indexBuffer = nil;
    p += 4;
    header->primType = *(uint32 *)p;
    p += 4;
    p += 16 *
         2;  // skip vertex streams, they're repeated with the vertex buffers
    header->useOffsets = *(bool32 *)p;
    p += 4;
    header->vertexDeclaration = nil;
    p += 4;
    header->totalNumIndex = *(uint32 *)p;
    p += 4;
    header->totalNumVertex = *(uint32 *)p;
    p += 4;
    header->inst =
    ...
    for (uint32 i = 0; i < header->numMeshes; i++) {
        inst->numIndex = *(uint32 *)p;
        p += 4;
        inst->minVert = *(uint32 *)p;
        p += 4;
        uint32 matid = *(uint32 *)p;
        p += 4;
        inst->material = geometry->matList.materials[matid];
        inst->vertexAlpha = *(bool32 *)p;
        p += 4;
        inst->vertexShader = nil;
        p += 4;
        inst->baseIndex = 0;
        p += 4;
        inst->numVertices = *(uint32 *)p;
        p += 4;
        inst->startIndex = *(uint32 *)p;
        p += 4;
        inst->numPrimitives = *(uint32 *)p;
        p += 4;
        inst++;
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        s->vertexBuffer = (void *)(uintptr) * (uint32 *)p;
        p += 4;
        s->offset = 0;
        p += 4;
        s->stride = *(uint32 *)p;
        p += 4;
        s->geometryFlags = *(uint16 *)p;
        p += 2;
        s->managed = *p++;
        s->dynamicLock = *p++;

        if (s->vertexBuffer == nil) continue;
        // TODO: unset managed flag when using morph targets.
        //       also uses different buffer type and locks differently
        s->vertexBuffer = createVertexBuffer(s->stride * header->totalNumVertex,
                                             0, D3DPOOL_MANAGED);
    ...
    // TODO: somehow depends on number of streams used (baseIndex = minVert when
    // more than one)
    inst = header->inst;
    for (uint32 i = 0; i < header->numMeshes; i++) {
        inst->baseIndex = inst->minVert + header->vertexStream[0].offset /
...
Stream *writeNativeData(Stream *stream, int32 len, void *object, int32, int32) {
    ...
    uint8 *p = data;
    *(uint32 *)p = header->serialNumber;
    p += 4;
    *(uint32 *)p = header->numMeshes;
    p += 4;
    p += 4;  // skip index buffer
    *(uint32 *)p = header->primType;
    p += 4;
    p += 16 *
         2;  // skip vertex streams, they're repeated with the vertex buffers
    *(bool32 *)p = header->useOffsets;
    p += 4;
    p += 4;  // skip vertex declaration
    *(uint32 *)p = header->totalNumIndex;
    p += 4;
    *(uint32 *)p = header->totalNumVertex;
    p += 4;

    InstanceData *inst = header->inst;
    for (uint32 i = 0; i < header->numMeshes; i++) {
        *(uint32 *)p = inst->numIndex;
        p += 4;
        *(uint32 *)p = inst->minVert;
        p += 4;
        int32 matid = geometry->matList.findIndex(inst->material);
        *(int32 *)p = matid;
        p += 4;
        *(bool32 *)p = inst->vertexAlpha;
        p += 4;
        *(uint32 *)p = 0;
        p += 4;  // vertex shader
        *(uint32 *)p = inst->baseIndex;
        p += 4;  // not used but meh...
        *(uint32 *)p = inst->numVertices;
        p += 4;
        *(uint32 *)p = inst->startIndex;
        p += 4;
        *(uint32 *)p = inst->numPrimitives;
        p += 4;
        inst++;
    }
    ...
    for (int i = 0; i < 2; i++) {
        s = &header->vertexStream[i];
        p = data;
        *(uint32 *)p = s->vertexBuffer ? 0xbadeaffe : 0;
        p += 4;
        *(uint32 *)p = s->offset;
        p += 4;
        *(uint32 *)p = s->stride;
        p += 4;
        *(uint16 *)p = s->geometryFlags;
        p += 2;
        *p++ = s->managed;
        *p++ = s->dynamicLock;
...
static void instance(rw::ObjPipeline *rwpipe, Atomic *atomic) {
    ...
    header->primType =
        meshh->flags == 1 ? D3DPT_TRIANGLESTRIP : D3DPT_TRIANGLELIST;
    header->useOffsets = 0;
    ...
    for (uint32 i = 0; i < header->numMeshes; i++) {
        ...
        inst->numPrimitives = header->primType == D3DPT_TRIANGLESTRIP
                                  ? inst->numIndex - 2
                                  : inst->numIndex / 3;
        if (inst->minVert == 0)
            ...
            for (uint32 j = 0; j < inst->numIndex; j++)
                indices[inst->startIndex + j] =
                    mesh->indices[j] - inst->minVert;
...
void defaultInstanceCB(Geometry *geo, InstanceDataHeader *header) {
    ...
    s->vertexBuffer = createVertexBuffer(header->totalNumVertex * s->stride, 0,
                                         D3DPOOL_MANAGED);

    // TODO: support both vertex buffers
    uint8 *verts = lockVertices(s->vertexBuffer, 0, 0, D3DLOCK_NOSYSLOCK);
    for (i = 0; dcl[i].usage != D3DDECLUSAGE_POSITION || dcl[i].usageIndex != 0;
         i++)
        ;
    instV3d(vertFormatMap[dcl[i].type], verts + dcl[i].offset,
            geo->morphTargets[0].vertices, header->totalNumVertex,
            header->vertexStream[dcl[i].stream].stride);

    if (isPrelit) {
        for (i = 0;
             dcl[i].usage != D3DDECLUSAGE_COLOR || dcl[i].usageIndex != 0; i++)
            ;
        InstanceData *inst = header->inst;
        ...
        while (n--) {
            ...
            inst->vertexAlpha = instColor(
                vertFormatMap[dcl[i].type],
                verts + dcl[i].offset + stride * inst->minVert,
                geo->colors + inst->minVert, inst->numVertices, stride);
            inst++;
        }
    }
    ...
    for (int32 n = 0; n < geo->numTexCoordSets; n++) {
        for (i = 0;
             dcl[i].usage != D3DDECLUSAGE_TEXCOORD || dcl[i].usageIndex != n;
             i++)
            ;
        instTexCoords(vertFormatMap[dcl[i].type], verts + dcl[i].offset,
                      geo->texCoords[n], header->totalNumVertex,
                      header->vertexStream[dcl[i].stream].stride);
    }

    if (hasNormals) {
        for (i = 0;
             dcl[i].usage != D3DDECLUSAGE_NORMAL || dcl[i].usageIndex != 0; i++)
            ;
        instV3d(vertFormatMap[dcl[i].type], verts + dcl[i].offset,
                geo->morphTargets[0].normals, header->totalNumVertex,
                header->vertexStream[dcl[i].stream].stride);
    }
    unlockVertices(s->vertexBuffer);
}
...
void defaultUninstanceCB(Geometry *geo, InstanceDataHeader *header) {
    ...
    uint8 *verts[2];
    verts[0] = lockVertices(header->vertexStream[0].vertexBuffer, 0, 0,
                            D3DLOCK_NOSYSLOCK);
    verts[1] = lockVertices(header->vertexStream[1].vertexBuffer, 0, 0,
                            D3DLOCK_NOSYSLOCK);
    ...
    for (i = 0; dcl[i].usage != D3DDECLUSAGE_POSITION || dcl[i].usageIndex != 0;
         i++)
        ;
    uninstV3d(vertFormatMap[dcl[i].type], geo->morphTargets[0].vertices,
              verts[dcl[i].stream] + dcl[i].offset, header->totalNumVertex,
              header->vertexStream[dcl[i].stream].stride);
    ...
    if (geo->flags & Geometry::PRELIT) {
        for (i = 0;
             dcl[i].usage != D3DDECLUSAGE_COLOR || dcl[i].usageIndex != 0; i++)
            ;
        uninstColor(vertFormatMap[dcl[i].type], geo->colors,
    ...
    for (int32 n = 0; n < geo->numTexCoordSets; n++) {
        for (i = 0;
             ...
             i++)
            ;
        uninstTexCoords(vertFormatMap[dcl[i].type], geo->texCoords[n],
                        verts[dcl[i].stream] + dcl[i].offset,
                        header->totalNumVertex,
                        header->vertexStream[dcl[i].stream].stride);
    }
    ...
    if (geo->flags & Geometry::NORMALS) {
        for (i = 0;
             dcl[i].usage != D3DDECLUSAGE_NORMAL || dcl[i].usageIndex != 0; i++)
            ;
        uninstV3d(vertFormatMap[dcl[i].type], geo->morphTargets[0].normals,
                  verts[dcl[i].stream] + dcl[i].offset, header->totalNumVertex,
                  header->vertexStream[dcl[i].stream].stride);
    }
...
Texture *readNativeTexture(Stream *stream) {
    ...
    */
    ...
    if (flags & 8) {
        ...
        raster =
            Raster::create(width, height, depth,
                           format | type | Raster::DONTALLOCATE, PLATFORM_D3D9);
        ext = PLUGINOFFSET(D3dRaster, raster, nativeRasterOffset);
        ext->format = d3dformat;
        ext->hasAlpha = flags & 1;
        ext->texture = createTexture(
            raster->width, raster->height,
            raster->format & Raster::MIPMAP ? numLevels : 1, ext->format);
        ...
        raster =
            Raster::create(width, height, depth, format | type, PLATFORM_D3D9);
        ext = PLUGINOFFSET(D3dRaster, raster, nativeRasterOffset);
    }
    tex->raster = raster;
...
void writeNativeTexture(Texture *tex, Stream *stream) {
    ...
    uint8 flags = 0;
    if (ext->hasAlpha) flags |= 1;
    // no automipmapgen and cube supported yet
    if (ext->customFormat) flags |= 8;
