...
void Clump::destroy(void) {
    ...
    if (f = this->getFrame(), f) f->destroyHierarchy();
    rwFree(this);
}

Clump *Clump::streamRead(Stream *stream) {
    ...
    if (frmlst.streamRead(stream) == nil) goto fail;
    ...
    if (version >= 0x30400) {
        ...
        if (numGeometries) {
            ...
            geometryList =
                (Geometry **)rwMalloc(sz, MEMDUR_FUNCTION | ID_CLUMP);
            if (geometryList == nil) {
        ...
        for (int32 i = 0; i < numGeometries; i++) {
            ...
            if (geometryList[i] == nil) goto failgeo;
        }
    }
    ...
    for (int32 i = 0; i < numCameras; i++) {
        ...
        if (cam == nil) goto failgeo;
        cam->setFrame(frmlst.frames[frm]);
        clump->addCamera(cam);
    }

    for (int32 i = 0; i < numGeometries; i++)
        if (geometryList[i]) geometryList[i]->destroy();
    rwFree(geometryList);
    rwFree(frmlst.frames);
    if (s_plglist.streamRead(stream, clump)) return clump;
...
bool Clump::streamWrite(Stream *stream) {
    ...
    frmlst.frames = (Frame **)rwMalloc(frmlst.numFrames * sizeof(Frame *),
                                       MEMDUR_FUNCTION | ID_CLUMP);
    ...
    FORLIST(lnk, this->lights) {
        ...
        int frm = findPointer(l->getFrame(), (void **)frmlst.frames,
                              frmlst.numFrames);
    ...
    FORLIST(lnk, this->cameras) {
        ...
        int frm = findPointer(c->getFrame(), (void **)frmlst.frames,
                              frmlst.numFrames);
        if (frm < 0) return false;
        writeChunkHeader(stream, ID_STRUCT, 4);
        stream->writeI32(frm);
...
uint32 Clump::streamGetSize(void) {
    uint32 size = 0;
    size += 12;                        // Struct
    size += 4;                         // numAtomics
    if (version > 0x33000) size += 8;  // numLights, numCameras
...
void Clump::render(void) {
    ...
    FORLIST(lnk, this->atomics) {
        ...
        if (a->object.object.flags & Atomic::RENDER) a->render();
    }
}
...
static void atomicSync(ObjectWithFrame *obj) {
    // TODO: interpolate
    obj->object.privateFlags |= Atomic::WORLDBOUNDDIRTY;
}

static void worldAtomicSync(ObjectWithFrame *obj) {
    Atomic *atomic = (Atomic *)obj;
    atomic->originalSync(obj);
}

Atomic *Atomic::create(void) {
    Atomic *atomic =
        (Atomic *)rwMalloc(s_plglist.size, MEMDUR_EVENT | ID_ATOMIC);
    if (atomic == nil) {
        RWERROR((ERR_ALLOC, s_plglist.size));
        return nil;
    }
...
Atomic *Atomic::clone() {
    ...
    if (atomic == nil) return nil;
    atomic->object.object.copy(&this->object.object);
    atomic->object.object.privateFlags |= WORLDBOUNDDIRTY;
    if (this->geometry) atomic->setGeometry(this->geometry, 0);
...
void Atomic::destroy(void) {
    s_plglist.destruct(this);
    if (this->geometry) this->geometry->destroy();
    if (this->clump) this->inClump.remove();
    this->setFrame(nil);
    rwFree(this);
}

void Atomic::removeFromClump(void) {
    if (this->clump) {
        this->inClump.remove();
        this->clump = nil;
    }
...
void Atomic::setGeometry(Geometry *geo, uint32 flags) {
    if (this->geometry) this->geometry->destroy();
    if (geo) geo->refCount++;
    this->geometry = geo;
    if (flags & SAMEBOUNDINGSPHERE) return;
    if (geo) {
...
Sphere *Atomic::getWorldBoundingSphere(void) {
...
Atomic *Atomic::streamReadClump(Stream *stream, FrameList_ *frameList,
                                Geometry **geometryList) {
    int32 buf[4];
    uint32 version;
    ...
    atomicRights[0] = 0;
    if (!s_plglist.streamRead(stream, atomic)) goto fail;
    if (atomicRights[0])
...
uint32 Atomic::streamGetSize(void) {
...
ObjPipeline *Atomic::getPipeline(void) {
    return this->pipeline ? this->pipeline
                          : engine->driver[platform]->defaultPipeline;
}

void Atomic::instance(void) {
    if (this->geometry->flags & Geometry::NATIVE) return;
    this->getPipeline()->instance(this);
    this->geometry->flags |= Geometry::NATIVE;
}

void Atomic::uninstance(void) {
    if (!(this->geometry->flags & Geometry::NATIVE)) return;
...
void Atomic::defaultRenderCB(Atomic *atomic) {
    atomic->getPipeline()->render(atomic);
}

// Atomic Rights plugin

static Stream *readAtomicRights(Stream *stream, int32, void *, int32, int32) {
    stream->read(atomicRights, 8);
    return stream;
}

static Stream *writeAtomicRights(Stream *stream, int32, void *object, int32,
                                 int32) {
...
static int32 getSizeAtomicRights(void *object, int32, int32) {
    Atomic *atomic = (Atomic *)object;
    if (atomic->pipeline == nil || atomic->pipeline->pluginID == 0) return 0;
    return 8;
}

void registerAtomicRightsPlugin(void) {
    Atomic::registerPlugin(0, ID_RIGHTTORENDER, nil, nil, nil);
    Atomic::registerPluginStream(ID_RIGHTTORENDER, readAtomicRights,
                                 writeAtomicRights, getSizeAtomicRights);
}
