...
Image *Image::create(int32 width, int32 height, int32 depth) {
...
void Image::allocate(void) {
    if (this->pixels == nil) {
        ...
        this->pixels =
            rwNewT(uint8, this->stride * this->height, MEMDUR_EVENT | ID_IMAGE);
        this->flags |= 1;
    }
    if (this->palette == nil) {
        if (this->depth == 4 || this->depth == 8)
            this->palette = rwNewT(uint8, (this->depth == 4 ? 16 : 256) * 4,
                                   MEMDUR_EVENT | ID_IMAGE);
        this->flags |= 2;
    }
}
...
void decompressDXT1(uint8 *adst, int32 w, int32 h, uint8 *src) {
...
void decompressDXT3(uint8 *adst, int32 w, int32 h, uint8 *src) {
    /* j loops through old texels
     * x and y loop through new texels */
    int32 x = 0, y = 0;
    uint32 c[4][4];
...
void decompressDXT5(uint8 *adst, int32 w, int32 h, uint8 *src) {
    /* j loops through old texels
     * x and y loop through new texels */
    int32 x = 0, y = 0;
    uint32 c[4][4];
...
void Image::setPixelsDXT(int32 type, uint8 *pixels) {
    switch (type) {
        case 1:
...
void Image::setPalette(uint8 *palette) {
    this->palette = palette;
    this->flags |= 2;
}

bool32 Image::hasAlpha(void) {
    uint8 ret = 0xFF;
    uint8 *pixels = this->pixels;
    if (this->depth == 24) return 0;
    if (this->depth == 32) {
...
void Image::unindex(void) {
    ...
    uint8 *npixels =
        rwNewT(uint8, nstride * this->height, MEMDUR_EVENT | ID_IMAGE);
    ...
    for (int32 y = 0; y < this->height; y++) {
        ...
        for (int32 x = 0; x < this->width; x++) {
            ...
            np += 3;
            if (ndepth == 32) *np++ = this->palette[*p * 4 + 3];
            p++;
        }
        line += this->stride;
        nline += nstride;
...
void Image::removeMask(void) {
    if (this->depth <= 8) {
        assert(this->palette);
        int32 pallen = 4 * (this->depth == 4 ? 16 : 256);
        for (int32 i = 0; i < pallen; i += 4) this->palette[i] = 0xFF;
        return;
    }
...
static char *rwstrdup(const char *s) {
    char *t;
...
void Image::setSearchPath(const char *path) {
    char *p, *end;
    rwFree(searchPaths);
    numSearchPaths = 0;
    if (path)
...
void Image::printSearchPath(void) {
    char *p = searchPaths;
...
char *Image::getFilename(const char *name) {
    FILE *f;
