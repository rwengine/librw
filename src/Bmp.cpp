...
void writeBMP(Image *image, const char *filename) {
    ...
    int32 pallen = image->depth > 8 ? 0 : image->depth == 4 ? 16 : 256;
    ...
    *p++ = 'B';
    *p++ = 'M';
    *(uint32 *)p = 0x36 + 4 * pallen + image->height * stride;
    p += 4;
    *(uint16 *)p = 0;
    p += 2;  // reserved
    *(uint16 *)p = 0;
    p += 2;
    *(uint32 *)p = 0x36 + 4 * pallen;
    p += 4;  // data offset
    ...
    *(int16 *)p = depth;
    p += 2;
    *(uint32 *)p = 0;
    p += 4;  // compression: none
    *(int32 *)p = 0;
    p += 4;  // size, not needed in our case
    *(int32 *)p = 2835;
    p += 4;  // x resolution, 72dpi
    *(int32 *)p = 2835;
    p += 4;  // y resolution
    *(int32 *)p = 0;
    p += 4;  // number of used palette colors: max
    *(int32 *)p = 0;
    p += 4;  // important pixels
