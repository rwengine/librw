...
// We allocate twice because we have to allocate the data separately for
// uninstancing
Geometry *Geometry::create(int32 numVerts, int32 numTris, uint32 flags) {
    Geometry *geo =
        (Geometry *)rwMalloc(s_plglist.size, MEMDUR_EVENT | ID_GEOMETRY);
    if (geo == nil) {
    ...
    if (geo->numTexCoordSets == 0)
        geo->numTexCoordSets =
            (geo->flags & TEXTURED) ? 1 : (geo->flags & TEXTURED2) ? 2 : 0;
    geo->numTriangles = numTris;
    geo->numVertices = numVerts;

    geo->colors = nil;
    for (int32 i = 0; i < 8; i++) geo->texCoords[i] = nil;
    geo->triangles = nil;
    ...
    if (!(geo->flags & NATIVE)) {
        ...
        if (geo->flags & PRELIT) sz += geo->numVertices * sizeof(RGBA);
...
Geometry *Geometry::streamRead(Stream *stream) {
    uint32 version;
    GeoStreamData buf;
    ...
    Geometry *geo =
        Geometry::create(buf.numVertices, buf.numTriangles, buf.flags);
    if (geo == nil) return nil;
    geo->addMorphTargets(buf.numMorphTargets - 1);
    if (version < 0x34000) stream->read(&surfProps, 12);

    if (!(geo->flags & NATIVE)) {
        if (geo->flags & PRELIT)
            stream->read(geo->colors, 4 * geo->numVertices);
        for (int32 i = 0; i < geo->numTexCoordSets; i++)
            stream->read(geo->texCoords[i], 2 * geo->numVertices * 4);
        for (int32 i = 0; i < geo->numTriangles; i++) {
            uint32 tribuf[2];
    ...
    for (int32 i = 0; i < geo->numMorphTargets; i++) {
        ...
        if (hasVertices) stream->read(m->vertices, 3 * geo->numVertices * 4);
        if (hasNormals) stream->read(m->normals, 3 * geo->numVertices * 4);
    }
    ...
    if (version < 0x34000) defaultSurfaceProps = surfProps;

    ret = MaterialList::streamRead(stream, &geo->matList);
    if (version < 0x34000) defaultSurfaceProps = reset;
    if (ret == nil) goto fail;
    if (s_plglist.streamRead(stream, geo)) return geo;
...
static uint32 geoStructSize(Geometry *geo) {
    uint32 size = 0;
    size += sizeof(GeoStreamData);
    if (version < 0x34000) size += 12;  // surface properties
    if (!(geo->flags & Geometry::NATIVE)) {
        if (geo->flags & geo->PRELIT) size += 4 * geo->numVertices;
        for (int32 i = 0; i < geo->numTexCoordSets; i++)
    ...
    for (int32 i = 0; i < geo->numMorphTargets; i++) {
        ...
        if (!(geo->flags & Geometry::NATIVE)) {
            if (m->vertices) size += 3 * geo->numVertices * 4;
...
bool Geometry::streamWrite(Stream *stream) {
    GeoStreamData buf;
    ...
    if (version < 0x34000) stream->write(fbuf, sizeof(fbuf));

    if (!(this->flags & NATIVE)) {
        if (this->flags & PRELIT)
            stream->write(this->colors, 4 * this->numVertices);
        for (int32 i = 0; i < this->numTexCoordSets; i++)
            stream->write(this->texCoords[i], 2 * this->numVertices * 4);
        for (int32 i = 0; i < this->numTriangles; i++) {
            uint32 tribuf[2];
            tribuf[0] = this->triangles[i].v[0] << 16 | this->triangles[i].v[1];
            tribuf[1] =
                this->triangles[i].v[2] << 16 | this->triangles[i].matId;
    ...
    for (int32 i = 0; i < this->numMorphTargets; i++) {
        ...
        if (!(this->flags & NATIVE)) {
            ...
            if (m->vertices)
                stream->write(m->vertices, 3 * this->numVertices * 4);
            if (m->normals)
                stream->write(m->normals, 3 * this->numVertices * 4);
        } else {
            stream->writeI32(0);
            stream->writeI32(0);
        }
...
uint32 Geometry::streamGetSize(void) {
    uint32 size = 0;
...
void Geometry::addMorphTargets(int32 n) {
    if (n == 0) return;
    ...
    if (!(this->flags & NATIVE)) {
        ...
        if (this->flags & NORMALS) sz += this->numVertices * sizeof(V3d);
    }

    // Memory layout: MorphTarget[n]; (vertices and normals)[n]
    MorphTarget *mts;
    if (this->numMorphTargets) {
        mts = (MorphTarget *)rwResize(this->morphTargets, n * sz,
                                      MEMDUR_EVENT | ID_GEOMETRY);
        this->morphTargets = mts;
        // Since we now have more morph targets than before, move the vertex
        // data up
    ...
    // Set up everything and initialize the bounding sphere for new morph
    // targets
    V3d *data = (V3d *)&mts[n];
    for (int32 i = 0; i < n; i++) {
        mts->parent = this;
...
void Geometry::calculateBoundingSphere(void) {
    for (int32 i = 0; i < this->numMorphTargets; i++) {
        MorphTarget *m = &this->morphTargets[i];
...
bool32 Geometry::hasColoredMaterial(void) {
...
void Geometry::allocateData(void) {
    // Geometry data
    // Pretty much copy pasted from ::create above
    int32 sz = this->numTriangles * sizeof(Triangle);
    if (this->flags & PRELIT) sz += this->numVertices * sizeof(RGBA);
    ...
    MorphTarget *mt =
        (MorphTarget *)rwResize(this->morphTargets, sz * this->numMorphTargets,
                                MEMDUR_EVENT | ID_GEOMETRY);
...
static int isDegenerate(uint16 *idx) {
...
void Geometry::generateTriangles(int8 *adc) {
    ...
    for (uint32 i = 0; i < header->numMeshes; i++) {
        ...
        if (header->flags == MeshHeader::TRISTRIP) {
            for (uint32 j = 0; j < m->numIndices - 2; j++) {
                if (!(adc && adcbits[j + 2]) && !isDegenerate(&m->indices[j]))
                    this->numTriangles++;
            }
        } else
...
static void dumpMesh(Mesh *m) {
    for (int32 i = 0; i < m->numIndices - 2; i++)
        ...
        //				m->indices[i+2]);
        //		else
        printf("%d %d %d\n", m->indices[i], m->indices[i + 1],
               m->indices[i + 2]);
}

void Geometry::buildMeshes(void) {
    Triangle *tri;
    Mesh *mesh;
    ...
    if ((this->flags & Geometry::TRISTRIP) == 0) {
        int32 *numIndices =
            rwNewT(int32, numMeshes, MEMDUR_FUNCTION | ID_GEOMETRY);
        ...
        for (int32 i = 0; i < numMeshes; i++) mesh[i].numIndices = 0;
        tri = this->triangles;
        for (int32 i = 0; i < this->numTriangles; i++) {
...
void Geometry::correctTristripWinding(void) {
    MeshHeader *header = this->meshHeader;
    ...
    uint16 *indices =
        rwNewT(uint16, header->totalIndices * 2, MEMDUR_FUNCTION | ID_GEOMETRY);
    ...
    memcpy(this->meshHeader->getMeshes()->indices, indices,
           this->meshHeader->totalIndices * 2);
    rwFree(indices);
}

void Geometry::removeUnusedMaterials(void) {
    if (this->meshHeader == nil) return;
    MeshHeader *mh = this->meshHeader;
    Mesh *m = mh->getMeshes();
    for (uint32 i = 0; i < mh->numMeshes; i++)
        if (m[i].indices == nil) return;
...
void MaterialList::init(void) {
    this->materials = nil;
    this->numMaterials = 0;
    this->space = 0;
}

void MaterialList::deinit(void) {
    if (this->materials) {
...
int32 MaterialList::appendMaterial(Material *mat) {
    Material **ml;
    int32 space;
...
int32 MaterialList::findIndex(Material *mat) {
    for (int32 i = 0; i < this->numMaterials; i++)
        if (this->materials[i] == mat) return i;
    return -1;
}

MaterialList *MaterialList::streamRead(Stream *stream, MaterialList *matlist) {
    int32 *indices = nil;
    int32 numMat;
    ...
    if (numMat == 0) return matlist;
    matlist->materials =
        rwMallocT(Material *, numMat, MEMDUR_EVENT | ID_MATERIAL);
    if (matlist->materials == nil) goto fail;
...
bool MaterialList::streamWrite(Stream *stream) {
    ...
    for (int32 i = 0; i < this->numMaterials; i++) {
        for (int32 j = i - 1; j >= 0; j--)
            if (this->materials[i] == this->materials[j]) goto found;
...
uint32 MaterialList::streamGetSize(void) {
...
Material *Material::create(void) {
    Material *mat =
        (Material *)rwMalloc(s_plglist.size, MEMDUR_EVENT | ID_MATERIAL);
    if (mat == nil) {
        RWERROR((ERR_ALLOC, s_plglist.size));
        return nil;
    }
...
Material *Material::clone(void) {
    ...
    if (this->texture) mat->setTexture(this->texture);
...
Material *Material::streamRead(Stream *stream) {
    ...
    materialRights[0] = 0;
    if (!s_plglist.streamRead(stream, mat)) goto fail;
    if (materialRights[0])
...
bool Material::streamWrite(Stream *stream) {
    MatStreamData buf;

    writeChunkHeader(stream, ID_MATERIAL, this->streamGetSize());
    writeChunkHeader(stream, ID_STRUCT,
                     sizeof(MatStreamData) + (rw::version >= 0x30400 ? 12 : 0));
    ...
    if (this->texture) this->texture->streamWrite(stream);

    s_plglist.streamWrite(stream, this);
    return true;
}

uint32 Material::streamGetSize(void) {
    uint32 size = 0;
    size += 12 + sizeof(MatStreamData);
    if (rw::version >= 0x30400) size += 12;
    if (this->texture) size += 12 + this->texture->streamGetSize();
    size += 12 + s_plglist.streamGetSize(this);
    return size;
}
...
static Stream *readMaterialRights(Stream *stream, int32, void *, int32, int32) {
    stream->read(materialRights, 8);
    //	printf("materialrights: %X %X\n", materialRights[0], materialRights[1]);
    return stream;
}

static Stream *writeMaterialRights(Stream *stream, int32, void *object, int32,
                                   int32) {
...
static int32 getSizeMaterialRights(void *object, int32, int32) {
    Material *material = (Material *)object;
...
void registerMaterialRightsPlugin(void) {
    ...
    Material::registerPluginStream(ID_RIGHTTORENDER, readMaterialRights,
                                   writeMaterialRights, getSizeMaterialRights);
}
