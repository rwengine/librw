...
MeshHeader *Geometry::allocateMeshes(int32 numMeshes, uint32 numIndices,
                                     bool32 noIndices) {
    uint32 sz;
    MeshHeader *mh;
    Mesh *m;
    ...
    if (!noIndices) sz += numIndices * sizeof(uint16);
    if (this->meshHeader) {
        oldNumMeshes = this->meshHeader->numMeshes;
        mh = (MeshHeader *)rwResize(this->meshHeader, sz,
...
void MeshHeader::setupIndices(void) {
    int32 i;
    uint16 *indices;
    Mesh *m;
    m = this->getMeshes();
...
static Stream *readMesh(Stream *stream, int32 len, void *object, int32, int32) {
    ...
    bool32 hasData =
        len > sizeof(MeshHeaderStream) + mhs.numMeshes * sizeof(MeshStream);
    ...
    for (uint32 i = 0; i < mh->numMeshes; i++) {
        ...
        if (geo->flags & Geometry::NATIVE) {
            ...
            if (hasData) {
                ...
                stream->read(mesh->indices, mesh->numIndices * 2);
            }
        } else {
            mesh->indices = indices;
            indices += mesh->numIndices;
            ...
            for (; numIndices > 0; numIndices -= 256) {
                ...
                for (int32 j = 0; j < n; j++) ind[j] = indbuf[j];
                ind += n;
            }
        }
        mesh++;
...
static Stream *writeMesh(Stream *stream, int32, void *object, int32, int32) {
    MeshHeaderStream mhs;
    MeshStream ms;
    int32 indbuf[256];
    Geometry *geo = (Geometry *)object;
    ...
    for (uint32 i = 0; i < geo->meshHeader->numMeshes; i++) {
        ...
        if (geo->flags & Geometry::NATIVE) {
            ...
            if (geo->instData->platform == PLATFORM_WDGL)
                stream->write(mesh->indices, mesh->numIndices * 2);
        } else {
            uint16 *ind = mesh->indices;
            int32 numIndices = mesh->numIndices;
            for (; numIndices > 0; numIndices -= 256) {
                int32 n = numIndices < 256 ? numIndices : 256;
                for (int32 j = 0; j < n; j++) indbuf[j] = ind[j];
...
static int32 getSizeMesh(void *object, int32, int32) {
    Geometry *geo = (Geometry *)object;
    if (geo->meshHeader == nil) return -1;
...
uint32 MeshHeader::guessNumTriangles(void) {
...
static void *destroyNativeData(void *object, int32 offset, int32 size) {
    Geometry *geometry = (Geometry *)object;
    if (geometry->instData == nil) return object;
...
static Stream *readNativeData(Stream *stream, int32 len, void *object, int32 o,
                              int32 s) {
...
static Stream *writeNativeData(Stream *stream, int32 len, void *object, int32 o,
                               int32 s) {
    Geometry *geometry = (Geometry *)object;
    if (geometry->instData == nil) return stream;
    if (geometry->instData->platform == PLATFORM_PS2)
...
static int32 getSizeNativeData(void *object, int32 offset, int32 size) {
    Geometry *geometry = (Geometry *)object;
    if (geometry->instData == nil) return 0;
    if (geometry->instData->platform == PLATFORM_PS2)
