...
struct StripNode {
    ...
    uint8 stripVisited : 1; /* strip starting at this node was visited during
                               search */
...
static void printNode(StripMesh *sm, StripNode *n) {
    trace("%3ld: %3d  %3d.%d  %3d.%d %3d.%d || %3d %3d %3d\n", n - sm->nodes,
          n->stripId, n->e[0].node, n->e[0].isStrip, n->e[1].node,
          n->e[1].isStrip, n->e[2].node, n->e[2].isStrip, n->v[0], n->v[1],
          n->v[2]);
}

static void printLone(StripMesh *sm) {
    FORLIST(lnk, sm->loneNodes)
    printNode(sm, LLLinkGetData(lnk, StripNode, inlist));
}

static void printEnds(StripMesh *sm) {
    FORLIST(lnk, sm->endNodes)
    printNode(sm, LLLinkGetData(lnk, StripNode, inlist));
}

static void printSmesh(StripMesh *sm) {
    for (int32 i = 0; i < sm->numNodes; i++) printNode(sm, &sm->nodes[i]);
}

static void collectFaces(Geometry *geo, StripMesh *sm, uint16 m) {
    StripNode *n;
    Triangle *t;
    sm->numNodes = 0;
...
static GraphEdge findEdge(StripMesh *sm, int32 e[2]) {
    StripNode *n;
    GraphEdge ge = {0, 0, 0, 0};
    for (int32 i = 0; i < sm->numNodes; i++) {
        n = &sm->nodes[i];
        for (int32 j = 0; j < 3; j++) {
            if (n->e[j].isConnected) continue;
            if (e[0] == n->v[j] && e[1] == n->v[(j + 1) % 3]) {
                ge.node = i;
                // signal success
                ge.isConnected = 1;
                ge.otherEdge = j;
...
static void connectNodesPreserve(StripMesh *sm) {
    StripNode *n, *nn;
    int32 e[2];
    GraphEdge ge;
    for (int32 i = 0; i < sm->numNodes; i++) {
...
static int32 numConnections(StripNode *n) {
    return n->e[0].isConnected + n->e[1].isConnected + n->e[2].isConnected;
}

static int32 numStripEdges(StripNode *n) {
    return n->e[0].isStrip + n->e[1].isStrip + n->e[2].isStrip;
}

#define IsEnd(n) (numConnections(n) > 0 && numStripEdges(n) < 2)

/* Complement the strip-ness of an edge */
static void complementEdge(StripMesh *sm, GraphEdge *e) {
    e->isStrip = !e->isStrip;
...
static void extendStrip(StripMesh *sm, StripNode *start) {
    StripNode *n, *nn;
    n = start;
...
loop:
    ...
    for (int32 i = 0; i < 3; i++) {
        if (!n->e[i].isConnected) continue;
        nn = &sm->nodes[n->e[i].node];
        if (nn->stripId >= 0) continue;
...
static void buildStrips(StripMesh *sm) {
    StripNode *n;
    for (int32 i = 0; i < sm->numNodes; i++) {
        n = &sm->nodes[i];
        if (n->stripId >= 0) continue;
        n->stripId = i;
        extendStrip(sm, n);
    }
}

static StripNode *findTunnel(StripMesh *sm, StripNode *n) {
    ...
    for (;;) {
        for (int32 i = 0; i < 3; i++) {
            ...
            if (!n->e[i].isConnected || n->e[i].isStrip != edgetype) continue;
            nn = &sm->nodes[n->e[i].node];
            ...
            if (edgetype == 0 && n->stripId == nn->stripId) continue;

            isEnd = IsEnd(nn);

            /* Can't add end nodes to two lists, so skip. */
            if (isEnd && edgetype == 1) continue;
            ...
            /* Search complete. */
            if (isEnd && edgetype == 0) return nn;

            /* Found a valid node. */
            searchNodes.append(&nn->inlist);
        }
        if (searchNodes.isEmpty()) return nil;
...
static void resetGraph(StripMesh *sm) {
    StripNode *n;
    for (int32 i = 0; i < sm->numNodes; i++) {
        n = &sm->nodes[i];
        n->visited = 0;
...
static StripNode *walkStrip(StripMesh *sm, StripNode *start) {
    StripNode *n, *nn;
    int32 last;
    ...
    for (;; n = nn) {
        n->visited = 0;
        n->stripVisited = 0;
        if (n->isEnd) n->inlist.remove();
        n->isEnd = 0;

        if (IsEnd(n) && n != start) return n;

        /* find next node */
        nn = nil;
        for (int32 i = 0; i < 3; i++) {
            if (!n->e[i].isStrip || i == last) continue;
            nn = &sm->nodes[n->e[i].node];
        ...
        // trace("    next: ");
        // printNode(sm, nn);
        if (nn == nil) return nil;
    }
}

static void applyTunnel(StripMesh *sm, StripNode *end, StripNode *start) {
    LinkList tmplist;
    StripNode *n, *nn;
...
static void tunnel(StripMesh *sm) {
    StripNode *n, *nn;
...
static int getNextEdge(StripNode *n, int32 last) {
    int32 i;
    for (i = 0; i < 3; i++)
        if (n->e[i].isStrip && i != last) return i;
    return -1;
}
...
static void makeMesh(StripMesh *sm, Mesh *m) {
    int32 i, j;
    ...
    m->indices =
        rwNewT(uint16, sm->numNodes * 5, MEMDUR_FUNCTION | ID_GEOMETRY);
    ...
    FORLIST(lnk, sm->endNodes) {
        ...
        if (n->stripId != (n - sm->nodes)) continue;
        ...
        while (j >= 0) {
            ...
            trace("%d:%d%c %d %d %d\n", n - sm->nodes, m->numIndices,
                  even ? ' ' : '.', n->v[0], n->v[1], n->v[2]);
 ...
 */
void Geometry::buildTristrips(void) {
    int32 i;
    uint16 *indices;
    MeshHeader *header;
    Mesh *ms, *md;
    ...
    smesh.nodes =
        rwNewT(StripNode, this->numTriangles, MEMDUR_FUNCTION | ID_GEOMETRY);
...
static void verifyMesh(Geometry *geo) {
    ...
    for (i = 0; i < geo->meshHeader->numMeshes; i++) {
        ...
        for (j = 0; j < mesh->numIndices - 2; j++) {
            ...
            b = mesh->indices[j + x];
            c = mesh->indices[j + 2];
            if (a >= geo->numVertices || b >= geo->numVertices ||
                c >= geo->numVertices) {
