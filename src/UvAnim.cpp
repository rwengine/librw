...
void UVAnimCustomData::destroy(Animation *anim) {
    this->refCount--;
    if (this->refCount <= 0) anim->destroy();
}

UVAnimDictionary *currentUVAnimDictionary;

UVAnimDictionary *UVAnimDictionary::create(void) {
    UVAnimDictionary *dict = (UVAnimDictionary *)rwMalloc(
        sizeof(UVAnimDictionary), MEMDUR_EVENT | ID_UVANIMATION);
    if (dict == nil) {
...
void UVAnimDictionary::destroy(void) {
...
void UVAnimDictionary::add(Animation *anim) {
    UVAnimDictEntry *de =
        rwNewT(UVAnimDictEntry, 1, MEMDUR_EVENT | ID_UVANIMDICT);
    de->anim = anim;
    this->animations.append(&de->inDict);
}

UVAnimDictionary *UVAnimDictionary::streamRead(Stream *stream) {
...
bool UVAnimDictionary::streamWrite(Stream *stream) {
...
uint32 UVAnimDictionary::streamGetSize(void) {
    uint32 size = 12 + 4;
    FORLIST(lnk, this->animations) {
        UVAnimDictEntry *de = UVAnimDictEntry::fromDict(lnk);
...
Animation *UVAnimDictionary::find(const char *name) {
    FORLIST(lnk, this->animations) {
        Animation *anim = UVAnimDictEntry::fromDict(lnk)->anim;
        UVAnimCustomData *custom = UVAnimCustomData::get(anim);
        if (strncmp_ci(custom->name, name, 32) == 0) return anim;
    }
    return nil;
}

static void uvAnimStreamRead(Stream *stream, Animation *anim) {
    UVAnimCustomData *custom = UVAnimCustomData::get(anim);
...
static void uvAnimStreamWrite(Stream *stream, Animation *anim) {
    UVAnimCustomData *custom = UVAnimCustomData::get(anim);
    UVAnimKeyFrame *frames = (UVAnimKeyFrame *)anim->keyframes;
    stream->writeI32(0);
...
static uint32 uvAnimStreamGetSize(Animation *anim) {
    return 4 + 32 + 8 * 4 + anim->numFrames * (4 + 6 * 4 + 4);
}

static void uvAnimLinearApplyCB(void *result, void *frame) {
...
static void uvAnimLinearInterpCB(void *out, void *in1, void *in2, float32 t,
                                 void *custom) {
...
static void uvAnimParamApplyCB(void *result, void *frame) {
    Matrix *m = (Matrix *)result;
    UVAnimInterpFrame *f = (UVAnimInterpFrame *)frame;
...
static void uvAnimParamInterpCB(void *out, void *in1, void *in2, float32 t,
                                void *custom) {
    UVAnimInterpFrame *intf = (UVAnimInterpFrame *)out;
    UVAnimKeyFrame *kf1 = (UVAnimKeyFrame *)in1;
    UVAnimKeyFrame *kf2 = (UVAnimKeyFrame *)in2;
...
static void registerUVAnimInterpolator(void) {
    // TODO: implement this fully

    // Linear
    AnimInterpolatorInfo *info =
        rwNewT(AnimInterpolatorInfo, 1, MEMDUR_GLOBAL | ID_UVANIMATION);
    info->id = 0x1C0;
...
static void *createUVAnim(void *object, int32 offset, int32) {
    UVAnim *uvanim;
...
static void *destroyUVAnim(void *object, int32 offset, int32) {
    UVAnim *uvanim;
    uvanim = PLUGINOFFSET(UVAnim, object, offset);
    for (int32 i = 0; i < 8; i++) {
...
static void *copyUVAnim(void *dst, void *src, int32 offset, int32) {
    ...
    for (int32 i = 0; i < 8; i++) {
        ...
        if (srcip) {
            ...
            UVAnimCustomData *custom = UVAnimCustomData::get(anim);
            dstip = AnimInterpolator::create(
                anim->getNumNodes(), anim->interpInfo->interpKeyFrameSize);
...
Animation *makeDummyAnimation(const char *name) {
...
static Stream *readUVAnim(Stream *stream, int32, void *object, int32 offset,
                          int32) {
    ...
    for (int32 i = 0; i < 8; i++) {
        if (mask & bit) {
            ...
            if (anim == nil) {
                ...
                if (currentUVAnimDictionary) currentUVAnimDictionary->add(anim);
            }
            UVAnimCustomData *custom = UVAnimCustomData::get(anim);
            AnimInterpolator *interp;
            interp = AnimInterpolator::create(
                anim->getNumNodes(), anim->interpInfo->interpKeyFrameSize);
    ...
    // TEMP
    if (uvanim->uv[0] == nil) uvanim->uv[0] = Matrix::create();
    if (uvanim->uv[1] == nil) uvanim->uv[1] = Matrix::create();
    return stream;
}

static Stream *writeUVAnim(Stream *stream, int32 size, void *object,
                           int32 offset, int32) {
    UVAnim *uvanim = PLUGINOFFSET(UVAnim, object, offset);
...
static int32 getSizeUVAnim(void *object, int32 offset, int32) {
    UVAnim *uvanim = PLUGINOFFSET(UVAnim, object, offset);
    int32 size = 0;
    for (int32 i = 0; i < 8; i++)
        if (uvanim->interp[i]) size += 32;
    return size ? size + 12 + 4 : 0;
}

static void *uvanimOpen(void *object, int32 offset, int32 size) {
    registerUVAnimInterpolator();
    return object;
}
static void *uvanimClose(void *object, int32 offset, int32 size) {
    return object;
}
...
void registerUVAnimPlugin(void) {
    ...
    uvAnimOffset =
        Material::registerPlugin(sizeof(UVAnim), ID_UVANIMATION, createUVAnim,
                                 destroyUVAnim, copyUVAnim);
    Material::registerPluginStream(ID_UVANIMATION, readUVAnim, writeUVAnim,
                                   getSizeUVAnim);
}

bool32 UVAnim::exists(Material *mat) {
    int32 i;
    UVAnim *uvanim = PLUGINOFFSET(UVAnim, mat, uvAnimOffset);
    for (i = 0; i < 8; i++)
        if (uvanim->interp[i]) return 1;
    return 0;
}

void UVAnim::addTime(Material *mat, float32 t) {
    int32 i;
    UVAnim *uvanim = PLUGINOFFSET(UVAnim, mat, uvAnimOffset);
    for (i = 0; i < 8; i++)
        if (uvanim->interp[i]) uvanim->interp[i]->addTime(t);
}

void UVAnim::applyUpdate(Material *mat) {
    int32 i, j;
    int32 uv;
    Matrix m;
    UVAnim *uvanim = PLUGINOFFSET(UVAnim, mat, uvAnimOffset);
    for (i = 0; i < 2; i++)
        if (uvanim->uv[i]) uvanim->uv[i]->setIdentity();
    m.setIdentity();
